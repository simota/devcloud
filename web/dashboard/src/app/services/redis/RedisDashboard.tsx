...
export function RedisDashboard({ service }: RedisDashboardProps): JSX.Element {
  ...
  function handleSelectDB(db: number): void {
    selectRedisDB(db)
      .then(() => {
        setActiveKey(undefined)
        setDetail(undefined)
        setDetailError(undefined)
        setMessage({ tone: 'success', text: `Switched to db${db}` })
        refresh()
      })
      .catch((error: Error) => setMessage({ tone: 'error', text: error.message }))
  }
      ...
      <div className="redis-workspace">
        <Panel title="Keys">
          <div className="redis-keys-toolbar">
            <KeyCount loading={state.status === 'loading'} keys={keys.length} total={statusPayload?.currentDBKeys} />
...
function RedisStatusBar({
  ...
  const databaseCount = status.databaseCount > 0 ? status.databaseCount : 1
  return (
    <header className="redis-status-bar">
      ...
      <span className="redis-status-cell">
        <span className="cell-label">Database</span>
        <select
          aria-label="Select Redis database"
          className="redis-db-select"
          disabled={databaseCount <= 1}
          onChange={(event) => onSelectDB(Number(event.target.value))}
          value={status.currentDB}
        >
          {Array.from({ length: databaseCount }, (_, db) => (
            <option key={db} value={db}>{`db${db}`}</option>
          ))}
        </select>
      </span>
      <span className="redis-status-cell">
        <span className="cell-label">{`Keys (db${status.currentDB})`}</span>
        <span className="cell-value">{status.currentDBKeys}</span>
      </span>
    </header>
  )
}
