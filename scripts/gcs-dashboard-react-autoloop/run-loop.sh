...
Current context:
- GCS dashboard API exists in internal/dashboard/server.go under /api/gcs/*.
- Legacy /gcs static HTML exists in internal/dashboard/gcs_static.go.
...
5. Remove or bypass legacy static /gcs only after React route is covered.
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/dashboard web/dashboard README.md docs scripts/gcs-e2e.sh scripts/gcs-dashboard-react-autoloop 2>/dev/null || true
