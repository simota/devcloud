...
Keep the GCS dashboard in the shared React/Vite dashboard shell while preserving the existing `/gcs` compatibility route and `/api/gcs/*` dashboard API behavior.
...
8. The `/gcs` compatibility route remains covered by the React route, embedded asset, dashboard API, and tests.
