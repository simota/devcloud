...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/bigquery-dashboard-management-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
Goal:
- Read scripts/bigquery-dashboard-management-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the BigQuery dashboard management UI.
- Preserve existing user changes and existing BigQuery compatibility behavior.
...
Current context:
- BigQuery React UI exists at web/dashboard/src/app/services/bigquery/BigQueryDashboard.tsx.
- BigQuery dashboard API exists in services/dashboard/server.rs under /api/bigquery/*.
- Existing BigQuery service supports datasets, tables, insertAll, query jobs, and query results.
- Existing dashboard routes live in web/dashboard/src/app/routes.tsx.

Recommended order:
1. Add typed dashboard API helpers for query, dataset/table creation, and insertAll.
2. Add SQL query runner with dry-run, max results, result table, selected JSON, and job reference.
3. Add guided dataset/table creation and row insert JSON editor with validation.
4. Add job detail/recent query metadata.
5. Update dashboard tests/E2E and docs.

Safety:
- Mutations must go through /api/bigquery/* or the local BigQuery API path, never direct storage calls.
- Do not persist row payloads, credentials, Authorization headers, bearer tokens, or full request bodies.
- Disabled BigQuery state must not expose active mutation controls.
- Never log credentials, Authorization headers, bearer tokens, full row payloads, or query parameter secrets.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/bigquery-dashboard-management-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-management-ui bash scripts/bigquery-dashboard-management-autoloop/verify.sh
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore services/dashboard web/dashboard README.md docs scripts/bigquery-e2e.sh scripts/bigquery-autoloop/verify.sh scripts/bigquery-dashboard-management-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(bigquery): advance dashboard management loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/bigquery-dashboard-management-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: BigQuery dashboard management UI loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/bigquery-dashboard-management-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] BigQuery dashboard management UI completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
