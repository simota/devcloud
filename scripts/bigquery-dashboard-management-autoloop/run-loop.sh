...
- BigQuery dashboard API exists in internal/dashboard/server.go under /api/bigquery/*.
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/dashboard web/dashboard README.md docs scripts/bigquery-e2e.sh scripts/bigquery-autoloop/verify.sh scripts/bigquery-dashboard-management-autoloop 2>/dev/null || true
