...
assert_query_runner_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'runBigQueryQuery|BigQueryQueryRunner|jobs.query|queryResult|queryJob|dryRun|useLegacySql' web/dashboard/src/app/services/bigquery services/dashboard README.md docs &&
    env -u RIPGREP_CONFIG_PATH rg -q '/api/bigquery/projects/.*/queries|/queries|query runner|SQL query' web/dashboard/src/app/services/bigquery services/dashboard README.md docs &&
    cargo test --workspace
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_management_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'createBigQueryDataset|createBigQueryTable|insertBigQueryRows|datasets.insert|tables.insert|tabledata.insertAll' web/dashboard/src/app/services/bigquery services/dashboard README.md docs &&
