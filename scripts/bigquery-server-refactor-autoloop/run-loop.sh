...
Goal:
- Read scripts/bigquery-server-refactor-autoloop/goal.md.
- Refactor internal/services/bigquery/server.go into smaller files without changing behavior.
- Preserve existing user changes and avoid feature work.
- Keep all moved code in package bigquery.
- Prefer mechanical movement over renaming or logic cleanup.
- Do not modify scripts/bigquery-server-refactor-autoloop/progress.md, state.env, runner.log, runner.jsonl, or done.md; the runner owns loop state.
...
- Run go test ./internal/services/bigquery.
- Keep VERIFY_STAGE=foundation bash scripts/bigquery-server-refactor-autoloop/verify.sh passing.
- Use VERIFY_STAGE=full bash scripts/bigquery-server-refactor-autoloop/verify.sh only before claiming DONE.
...
Use DONE only when VERIFY_STAGE=full bash scripts/bigquery-server-refactor-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  if [[ -z "$(git status --short -- . ':!scripts/bigquery-server-refactor-autoloop/runner.log' ':!scripts/bigquery-server-refactor-autoloop/runner.jsonl' ':!scripts/bigquery-server-refactor-autoloop/state.env' ':!scripts/bigquery-server-refactor-autoloop/state.env.sha256' ':!scripts/bigquery-server-refactor-autoloop/progress.md' ':!scripts/bigquery-server-refactor-autoloop/done.md')" ]]; then
    return 0
  fi
  git add . \
    ':!scripts/bigquery-server-refactor-autoloop/runner.log' \
    ':!scripts/bigquery-server-refactor-autoloop/runner.jsonl' \
    ':!scripts/bigquery-server-refactor-autoloop/state.env' \
    ':!scripts/bigquery-server-refactor-autoloop/state.env.sha256' \
    ':!scripts/bigquery-server-refactor-autoloop/progress.md' \
    ':!scripts/bigquery-server-refactor-autoloop/done.md'
  git commit -m "refactor(bigquery): split server implementation iteration ${iteration}" -m "Loop status: ${status}"
}
...
main() {
  ...
  for ((ITERATION=NEXT_ITERATION; ITERATION<=end_iteration; ITERATION++)); do
    ...
    if [[ "${status}" == "DONE" && "${verify_result}" == "passed" ]]; then
      ...
      log "[DONE] BigQuery server refactor completed"
