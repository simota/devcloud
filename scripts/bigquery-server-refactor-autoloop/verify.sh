...
package_tests() {
  go test ./internal/services/bigquery
}
...
gofmt_clean() {
  local files
  files="$(gofmt -l internal/services/bigquery/*.go)"
  [[ -z "${files}" ]]
}

server_line_budget() {
  local lines
  lines="$(wc -l < internal/services/bigquery/server.go | tr -d ' ')"
...
focused_file_layout() {
  local count
  count="$(find internal/services/bigquery -maxdepth 1 -type f -name '*.go' \
...
package_name_consistent() {
  for file in internal/services/bigquery/*.go; do
...
case "${VERIFY_STAGE}" in
  foundation)
    ...
    run_check "gofmt is clean for BigQuery package" gofmt_clean
    ;;
  shape)
