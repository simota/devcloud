...
assert_redis_config_shape() {
  if [[ ! -f internal/app/config.go ]]; then
    return 1
  fi
  env -u RIPGREP_CONFIG_PATH rg -q 'RedisServiceConfig|RedisPort|Redis +RedisAuthConfig' internal/app/config.go
}

assert_redis_service_pkg_shape() {
  test -d internal/services/redis &&
    test -f internal/services/redis/server.go &&
    env -u RIPGREP_CONFIG_PATH rg -q 'func NewServer' internal/services/redis/server.go
}

assert_managed_redis_shape() {
  test -f internal/app/managed_redis.go &&
    env -u RIPGREP_CONFIG_PATH rg -q 'redis-server|SIGTERM|--requirepass|--port' internal/app/managed_redis.go
}

assert_dashboard_redis_shape() {
  test -f internal/dashboard/redis_handlers.go &&
    env -u RIPGREP_CONFIG_PATH rg -q '/api/redis/status|/api/redis/keys|allowlist|Allowlist|allowedCommands' internal/dashboard/redis_handlers.go &&
    env -u RIPGREP_CONFIG_PATH rg -q 'redis' internal/dashboard/services.go
}
...
run_foundation_checks() {
  ...
  run_check "devcloud builds" go build ./...
  run_check "go test ./..." go test ./...
}

run_config_checks() {
  run_check "redis config shape" assert_redis_config_shape
  run_check "internal/app tests" go test ./internal/app/...
}
...
run_service_checks() {
  ...
  run_check "internal/services/redis tests" go test ./internal/services/redis/...
}
...
run_dashboard_static_checks() {
  ...
  run_check "internal/dashboard tests" go test ./internal/dashboard/...
}
...
run_managed_lifecycle_checks() {
  ...
  run_check "managed redis tests" go test ./internal/app/... -run 'ManagedRedis'
}
