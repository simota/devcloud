...
- `foundation`: design doc + script contract + `go build` + `go test ./...`
- `config`: foundation + `internal/app` tests + Redis config shape
- `redis-core`: previous + `internal/services/redis` tests + managed-redis shape
- `dashboard-static`: previous + `internal/dashboard` tests + dashboard handler/UI shape
