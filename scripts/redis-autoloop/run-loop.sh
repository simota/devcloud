...
- Use Go standard library unless a dependency is unavoidable and justified. The dashboard's in-process client may depend on `github.com/redis/go-redis/v9`.
...
- internal/app/config.go and internal/app/daemon.go are the expected config and lifecycle integration points; mirror the Redshift `managed`/`external` shape.
- internal/app/managed_postgres.go shows the existing managed child-process pattern (used by Redshift). Copy this shape for `managed_redis.go`.
- internal/dashboard/services.go and web/dashboard/src/app/services/* show dashboard registration patterns; `RedshiftDashboard.tsx` is the closest 3-pane layout reference (replace clusters→keys, catalog→inspector, query runner→command runner).
- internal/dashboard/redshift_handlers.go shows the in-process service wiring style; never call raw storage from dashboard handlers.
- Recent commits established a JSON contract: every array field must be initialized to []T{} on the Go side so it serializes as [] (not null) for the React client.
...
2. internal/services/redis/{server,config}.go with Run(ctx) that either spawns redis-server (managed) or validates external connectivity (external) and exposes a typed in-process client.
3. internal/app/managed_redis.go that owns the redis-server child process, propagates --port/--dir/--save/--requirepass, and shuts down with SIGTERM→timeout→SIGKILL.
4. Dashboard backend: internal/dashboard/redis_handlers.go with /api/redis/status, /api/redis/keys (SCAN paging), /api/redis/keys/{key}, /api/redis/command (allowlist), DELETE /api/redis/keys/{key}, POST /api/redis/keys/{key}/expire, DELETE /api/redis/keys (FLUSHDB only).
5. Dashboard service registry entry in internal/dashboard/services.go and shared SetRedis on *Server.
...
7. Tests: internal/services/redis/*_test.go (config validation, allowlist), internal/dashboard/redis_test.go (every endpoint returns [] not null on empty), internal/app/managed_redis_test.go (graceful shutdown, binary-missing error message).
