...
Goal:
- Read scripts/sqs-server-refactor-autoloop/goal.md.
- Refactor internal/services/sqs/server.go into smaller files without changing behavior.
- Preserve existing user changes and avoid feature work.
- Keep all moved code in package sqs.
- Prefer mechanical movement over renaming or logic cleanup.
- Do not modify scripts/sqs-server-refactor-autoloop/progress.md, state.env, runner.log, runner.jsonl, or done.md; the runner owns loop state.
...
- Run go test ./internal/services/sqs.
- Keep VERIFY_STAGE=foundation bash scripts/sqs-server-refactor-autoloop/verify.sh passing.
- Use VERIFY_STAGE=full bash scripts/sqs-server-refactor-autoloop/verify.sh only before claiming DONE.
...
Use DONE only when VERIFY_STAGE=full bash scripts/sqs-server-refactor-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  if [[ -z "$(git status --short -- . ':!scripts/sqs-server-refactor-autoloop/runner.log' ':!scripts/sqs-server-refactor-autoloop/runner.jsonl' ':!scripts/sqs-server-refactor-autoloop/state.env' ':!scripts/sqs-server-refactor-autoloop/state.env.sha256' ':!scripts/sqs-server-refactor-autoloop/progress.md' ':!scripts/sqs-server-refactor-autoloop/done.md')" ]]; then
    return 0
  fi
  git add . \
    ':!scripts/sqs-server-refactor-autoloop/runner.log' \
    ':!scripts/sqs-server-refactor-autoloop/runner.jsonl' \
    ':!scripts/sqs-server-refactor-autoloop/state.env' \
    ':!scripts/sqs-server-refactor-autoloop/state.env.sha256' \
    ':!scripts/sqs-server-refactor-autoloop/progress.md' \
    ':!scripts/sqs-server-refactor-autoloop/done.md'
  git commit -m "refactor(sqs): split server implementation iteration ${iteration}" -m "Loop status: ${status}"
}
...
main() {
  ...
  for ((ITERATION=NEXT_ITERATION; ITERATION<=end_iteration; ITERATION++)); do
    ...
    if [[ "${status}" == "DONE" && "${verify_result}" == "passed" ]]; then
      ...
      log "[DONE] SQS server refactor completed"
