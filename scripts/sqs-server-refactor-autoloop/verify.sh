...
server_line_budget() {
  local lines
  lines="$(wc -l < services/sqs/server.rs | tr -d ' ')"
...
focused_file_layout() {
  local count
  count="$(find services/sqs -maxdepth 1 -type f -name '*.rs' \
...
package_name_consistent() {
  for file in services/sqs/*.rs; do
...
case "${VERIFY_STAGE}" in
  foundation)
    ...
    run_check "rustfmt is clean for SQS package" rustfmt_clean
    ;;
  shape)
