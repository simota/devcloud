...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# DynamoDB Dashboard Advanced Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: DynamoDB dashboard advanced UI loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-advanced-ui
...
echo "[OK] DynamoDB dashboard advanced autoloop bootstrap complete"
echo "Run: bash scripts/dynamodb-dashboard-advanced-autoloop/run-loop.sh"
