...
assert_pagination_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'LastEvaluatedKey|ExclusiveStartKey|nextPage|previousPage|pageHistory|pagination' web/dashboard/src/app/services/dynamodb internal/dashboard scripts &&
    ...
    go test ./internal/dashboard -run 'TestDynamoDB.*Pagination|TestDynamoDB.*Query|TestDynamoDB.*Scan' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_saved_recent_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'saved query|savedQuery|recent operation|recentOperation|operationHistory|localStorage' web/dashboard/src/app/services/dynamodb internal/dashboard README.md docs &&
    (cd web/dashboard && npm run typecheck && npm run build)
}
...
assert_delete_confirmation_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'two-step|second step|Confirm delete|DeleteItem confirmation|DeleteTable confirmation|confirmation text' web/dashboard/src/app/services/dynamodb README.md docs &&
    go test ./internal/dashboard -run 'TestDynamoDB.*Delete|TestDynamoDB.*Confirmation' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}
