...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-advanced-ui}"
...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/dynamodb-dashboard-advanced-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-advanced-ui}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/dynamodb-dashboard-advanced-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the DynamoDB dashboard advanced UI.
- Preserve existing user changes and completed DynamoDB management UI behavior.
...
Current context:
- DynamoDB metadata inspector, key lookup, guarded management flows, Query/Scan, E2E, and docs are already implemented.
- Existing files: web/dashboard/src/app/services/dynamodb/{DynamoDBDashboard.tsx,api.ts,types.ts}, services/dashboard/server.rs, services/dashboard/server_test.rs, scripts/dynamodb-e2e.sh, README.md.

Recommended order:
1. Query/Scan pagination UI/API.
2. Saved query or recent operation history without sensitive payload persistence.
3. CreateTable wizard and JSON validation.
4. Improved two-step destructive confirmation.
5. E2E/docs updates.

Safety:
- Do not persist sensitive item payloads in history.
- DeleteItem/DeleteTable must remain explicitly confirmed.
...
Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/dynamodb-dashboard-advanced-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-advanced-ui bash scripts/dynamodb-dashboard-advanced-autoloop/verify.sh
...
Use DONE only when the final full-advanced-ui gate passes completely.
PROMPT
}
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore services/dashboard web/dashboard README.md docs scripts/dynamodb-e2e.sh scripts/dynamodb-dashboard-advanced-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(dynamodb): advance dashboard advanced loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/dynamodb-dashboard-advanced-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: DynamoDB dashboard advanced UI loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/dynamodb-dashboard-advanced-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] DynamoDB dashboard advanced UI completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
