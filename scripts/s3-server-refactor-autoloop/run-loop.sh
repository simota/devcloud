...
Goal:
- Read scripts/s3-server-refactor-autoloop/goal.md.
- Refactor internal/services/s3/server.go into smaller files without changing behavior.
- Preserve existing user changes and avoid feature work.
- Keep all moved code in package s3.
- Prefer mechanical movement over renaming or logic cleanup.
- Do not modify scripts/s3-server-refactor-autoloop/progress.md, state.env, runner.log, runner.jsonl, or done.md; the runner owns loop state.

Suggested order:
1. Move response/listing/select/helper code.
...
- Run go test ./internal/services/s3.
- Keep VERIFY_STAGE=foundation bash scripts/s3-server-refactor-autoloop/verify.sh passing.
- Use VERIFY_STAGE=full bash scripts/s3-server-refactor-autoloop/verify.sh only before claiming DONE.
...
Use DONE only when VERIFY_STAGE=full bash scripts/s3-server-refactor-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  if [[ -z "$(git status --short -- . ':!scripts/s3-server-refactor-autoloop/runner.log' ':!scripts/s3-server-refactor-autoloop/runner.jsonl' ':!scripts/s3-server-refactor-autoloop/state.env' ':!scripts/s3-server-refactor-autoloop/state.env.sha256' ':!scripts/s3-server-refactor-autoloop/progress.md' ':!scripts/s3-server-refactor-autoloop/done.md')" ]]; then
    return 0
  fi
  git add . \
    ':!scripts/s3-server-refactor-autoloop/runner.log' \
    ':!scripts/s3-server-refactor-autoloop/runner.jsonl' \
    ':!scripts/s3-server-refactor-autoloop/state.env' \
    ':!scripts/s3-server-refactor-autoloop/state.env.sha256' \
    ':!scripts/s3-server-refactor-autoloop/progress.md' \
    ':!scripts/s3-server-refactor-autoloop/done.md'
  git commit -m "refactor(s3): split server implementation iteration ${iteration}" -m "Loop status: ${status}"
}
...
main() {
  ...
  for ((ITERATION=NEXT_ITERATION; ITERATION<=end_iteration; ITERATION++)); do
    ...
    if [[ "${status}" == "DONE" && "${verify_result}" == "passed" ]]; then
      ...
      log "[DONE] S3 server refactor completed"
