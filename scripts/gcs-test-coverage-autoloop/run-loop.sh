...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-test-coverage}"
...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/gcs-test-coverage-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-test-coverage}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/gcs-test-coverage-autoloop/goal.md.
- Implement the next smallest safe slice toward strengthening GCS test coverage.
- Preserve existing user changes and existing GCS compatibility/dashboard behavior.
...
Current context:
- GCS service tests live in internal/services/gcs/server_test.go.
- GCS dashboard API tests live in internal/dashboard/server_test.go.
- GCS E2E coverage lives in scripts/gcs-e2e.sh.
- Existing GCS service supports bucket/object CRUD, object metadata, generation/metageneration, listing filters, downloads, and resumable upload sessions.

Recommended order:
1. Add GCS service edge tests for invalid bucket/object behavior, metadata/content headers, generation/metageneration, listing pagination/filtering, resumable upload errors, delete not found, and download links.
2. Add dashboard GCS API tests for validation, disabled service responses, metadata propagation, upload session listing/deletion, mutation safety, and destructive confirmations.
3. Extend scripts/gcs-e2e.sh or docs to cover GCS management workflows and test intent.
4. Raise GCS service coverage to at least 72.0% and dashboard package coverage to at least 70.0%.
5. Keep tests deterministic and behavior-focused.

Safety:
- Prefer test-only changes. Production changes are allowed only when a new test exposes a real defect.
- Do not log object bodies, credentials, Authorization headers, bearer tokens, upload URLs containing secrets, or full request bodies.
- Do not add arbitrary sleeps or external Google Cloud dependencies.
- Isolate temporary state per test.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/gcs-test-coverage-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-test-coverage bash scripts/gcs-test-coverage-autoloop/verify.sh
...
Use DONE only when the final full-test-coverage gate passes completely.
PROMPT
}
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/services/gcs internal/dashboard README.md docs scripts/gcs-e2e.sh scripts/gcs-autoloop/verify.sh scripts/gcs-dashboard-react-autoloop/verify.sh scripts/gcs-test-coverage-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(gcs): advance test coverage loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/gcs-test-coverage-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: GCS test coverage loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/gcs-test-coverage-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] GCS test coverage completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
