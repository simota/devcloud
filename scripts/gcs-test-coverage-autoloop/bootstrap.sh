...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# GCS Test Coverage Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: GCS test coverage loop initialized."
  } > "${tmp_progress}"
...
echo "[OK] GCS test coverage autoloop bootstrap complete"
echo "Run: bash scripts/gcs-test-coverage-autoloop/run-loop.sh"
