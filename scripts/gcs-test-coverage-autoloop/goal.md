...
2. `internal/services/gcs/server_test.go` gains targeted tests for GCS edge/error behavior across bucket, object, metadata, generation/metageneration, resumable upload sessions, pagination, and delete flows.
3. `internal/dashboard/server_test.go` gains targeted tests for `/api/gcs/*` dashboard API validation, disabled-state behavior, mutation safety, metadata propagation, upload session listing/deletion, and destructive behavior where applicable.
...
- Prefer small, behavior-focused Go tests matching the existing `testing` style.
...
- Keep GCS tests scoped to `internal/services/gcs`, `internal/dashboard`, `scripts/gcs-e2e.sh`, README/docs, and this loop directory.
