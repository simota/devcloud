...
assert_service_edge_tests() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Invalid|Error|Metadata|Generation|Metageneration|Upload|Session|Pagination|Prefix|Delete|NotFound|Header' internal/services/gcs/server_test.go &&
    go test ./internal/services/gcs -run 'Test.*(Invalid|Error|Metadata|Generation|Metageneration|Upload|Session|Pagination|Prefix|Delete|NotFound|Header)' -count=1
}

assert_dashboard_api_tests() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Test.*GCS.*(Dashboard|API|Disabled|Invalid|Upload|Session|Delete|Metadata|Generation|Confirmation)' internal/dashboard/server_test.go &&
    env -u RIPGREP_CONFIG_PATH rg -q '/api/gcs|disabled|invalid|metadata|upload|session|generation|delete|confirmation' internal/dashboard/server_test.go &&
    go test ./internal/dashboard -run 'Test.*GCS.*(Dashboard|API|Disabled|Invalid|Upload|Session|Delete|Metadata|Generation|Confirmation)' -count=1
}
...
assert_e2e_docs_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'GCS test coverage|bucket create|bucket delete|object upload|object delete|metadata|generation|metageneration|upload session|resumable|CreateBucket|DeleteObject' README.md docs scripts/gcs-e2e.sh scripts/gcs-test-coverage-autoloop 2>/dev/null &&
    go test ./internal/services/gcs ./internal/dashboard -count=1
}

coverage_value() {
  local package="$1"
  go test -cover "${package}" | awk '/coverage:/{gsub("%","",$5); print $5; exit}'
}
...
assert_coverage_thresholds() {
  assert_min_coverage ./internal/services/gcs 72.0 &&
    assert_min_coverage ./internal/dashboard 70.0
}
