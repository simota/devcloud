...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# BigQuery Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: BigQuery implementation loop initialized."
  } > "${tmp_progress}"
...
echo "[OK] BigQuery autoloop bootstrap complete"
echo "Run: bash scripts/bigquery-autoloop/run-loop.sh"
