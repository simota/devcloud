  ...
  until curl -fsS "${url}" >/dev/null 2>&1; do
    if [[ -n "${DEV_PID}" ]] && ! kill -0 "${DEV_PID}" 2>/dev/null; then
      if [[ -n "${TMP_DIR}" && -f "${TMP_DIR}/devcloud-up.log" ]]; then
        sed 's/^/[bigquery-verify] devcloud: /' "${TMP_DIR}/devcloud-up.log" >&2
      fi
      return 1
    fi
...
assert_bigquery_config_shape() {
  env -u RIPGREP_CONFIG_PATH rg -q 'bigqueryPort|services\\.bigquery|auth\\.bigquery|BigQuery|bigquery' rust docs &&
...
case "${VERIFY_STAGE}" in
  foundation)
    ...
    run_check "CLI help works" cargo run -p devcloud-orchestrator -- help
    run_check "all Rust tests pass" cargo test --workspace
    ;;
  config)
