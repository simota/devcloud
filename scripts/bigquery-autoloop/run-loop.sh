...
preflight() {
  ...
  if [[ ! -f "docs/design-bigquery-compat.md" ]]; then
    log "[PREFLIGHT:FAIL] Missing docs/design-bigquery-compat.md"
    exit 1
  fi
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/bigquery-autoloop/recover.sh"
      exit 1
    fi
  fi
...
Goal:
- Read scripts/bigquery-autoloop/goal.md and docs/design-bigquery-compat.md.
- Implement the next smallest safe slice toward completing the BigQuery-compatible server.
- Preserve existing user changes.
- Do not modify scripts/bigquery-autoloop/progress.md, state.env, state.env.sha256, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
...
- Never log credentials, Authorization headers, bearer tokens, request bodies, row payloads, or unsanitized query text.

Current repository context:
- docs/design-bigquery-compat.md defines the BigQuery compatibility target.
- docs/design-s3-compat.md and docs/design-gcs-compat.md show the staged compatibility-document pattern.
...
- The current fast gate is: VERIFY_STAGE=foundation bash scripts/bigquery-autoloop/verify.sh
- The final acceptance gate is: VERIFY_STAGE=full bash scripts/bigquery-autoloop/verify.sh

Recommended implementation order:
1. Config and daemon wiring for services.bigquery, auth.bigquery, and server.bigqueryPort.
2. Minimal BigQuery REST v2 endpoint, project discovery, relaxed bearer-token handling, and compatible JSON errors.
3. Dataset catalog: datasets.insert, datasets.get, datasets.list, datasets.patch, datasets.delete.
4. Table catalog: tables.insert, tables.get, tables.list, tables.patch, tables.delete, schema validation basics.
5. Row ingestion and listing: tabledata.insertAll, tabledata.list, pagination-compatible response fields.
6. Query job MVP: jobs.query, jobs.insert query jobs, jobs.get, jobs.getQueryResults for the documented GoogleSQL subset.
7. Local GCS-backed load, extract, and copy job skeletons.
8. Strict auth mode and safe request logging.
9. Dashboard service registry, /dashboard/bigquery page, and /api/bigquery visibility.
...
Use DONE only when VERIFY_STAGE=full bash scripts/bigquery-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  git commit -m "feat(bigquery): advance implementation loop iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/bigquery-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: BigQuery-compatible server loop completed with verification evidence.
EOF
      log "[DONE] BigQuery-compatible server loop completed"
      exit 0
    fi
  done
