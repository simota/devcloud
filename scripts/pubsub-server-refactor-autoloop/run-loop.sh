...
Goal:
- Read scripts/pubsub-server-refactor-autoloop/goal.md.
- Refactor internal/services/pubsub/server.go into smaller files without changing behavior.
- Preserve existing user changes and avoid feature work.
- Keep all moved code in package pubsub.
- Prefer mechanical movement over renaming or logic cleanup.
- Do not modify scripts/pubsub-server-refactor-autoloop/progress.md, state.env, runner.log, runner.jsonl, or done.md; the runner owns loop state.
- Do not change internal/services/pubsub/grpc.go unless strictly required by compiler dependencies.

Suggested order:
1. Move pure types and response helpers.
...
6. Split tests only after source movement is stable.

Required checks:
- Run gofmt on changed Go files.
- Run go test ./internal/services/pubsub.
- Keep VERIFY_STAGE=foundation bash scripts/pubsub-server-refactor-autoloop/verify.sh passing.
- Use VERIFY_STAGE=full bash scripts/pubsub-server-refactor-autoloop/verify.sh only before claiming DONE.
...
Use DONE only when VERIFY_STAGE=full bash scripts/pubsub-server-refactor-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  if [[ -z "$(git status --short -- . ':!scripts/pubsub-server-refactor-autoloop/runner.log' ':!scripts/pubsub-server-refactor-autoloop/runner.jsonl' ':!scripts/pubsub-server-refactor-autoloop/state.env' ':!scripts/pubsub-server-refactor-autoloop/state.env.sha256' ':!scripts/pubsub-server-refactor-autoloop/progress.md' ':!scripts/pubsub-server-refactor-autoloop/done.md')" ]]; then
    return 0
  fi
  git add . \
    ':!scripts/pubsub-server-refactor-autoloop/runner.log' \
    ':!scripts/pubsub-server-refactor-autoloop/runner.jsonl' \
    ':!scripts/pubsub-server-refactor-autoloop/state.env' \
    ':!scripts/pubsub-server-refactor-autoloop/state.env.sha256' \
    ':!scripts/pubsub-server-refactor-autoloop/progress.md' \
    ':!scripts/pubsub-server-refactor-autoloop/done.md'
  git commit -m "refactor(pubsub): split server implementation iteration ${iteration}" -m "Loop status: ${status}"
}
...
main() {
  ...
  for ((ITERATION=NEXT_ITERATION; ITERATION<=end_iteration; ITERATION++)); do
    ...
    if [[ "${status}" == "DONE" && "${verify_result}" == "passed" ]]; then
      ...
      log "[DONE] Pub/Sub server refactor completed"
