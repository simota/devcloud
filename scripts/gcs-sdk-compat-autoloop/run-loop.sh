...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/gcs-sdk-compat-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
Goal:
- Read scripts/gcs-sdk-compat-autoloop/goal.md.
- Implement the next smallest safe slice toward GCS SDK compatibility E2E coverage.
- Preserve existing user changes and existing GCS REST/dashboard behavior.
...
Current context:
- Existing REST GCS E2E is scripts/gcs-e2e.sh.
- GCS service implementation is internal/services/gcs.
- The new target should prove official Google Cloud Storage SDK client compatibility via local endpoint override only.

Recommended order:
1. Add scripts/gcs-sdk-e2e.sh that starts devcloud in a temp workspace with isolated ports and GCS enabled.
2. Add a temporary Go SDK smoke program inside the script using cloud.google.com/go/storage.
3. Verify bucket create/list/get, object upload, metadata/attrs, download, object list, object delete, and bucket delete.
4. Update README or docs with the SDK E2E command and compatibility target.
5. Keep scripts/gcs-e2e.sh, VERIFY_STAGE=full bash scripts/gcs-autoloop/verify.sh, and go test ./... green.

Safety:
- Do not call real Google Cloud.
- Do not require gcloud auth, production credentials, or external emulators.
- Do not log credentials, Authorization headers, bearer tokens, signed URLs, or sensitive object payloads.
- Keep all temp files isolated and clean up the devcloud process.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/gcs-sdk-compat-autoloop/verify.sh
- SDK gate: VERIFY_STAGE=sdk-e2e bash scripts/gcs-sdk-compat-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-sdk-compat bash scripts/gcs-sdk-compat-autoloop/verify.sh
...
autocommit_if_requested() {
  ...
  git add README.md docs scripts/gcs-sdk-e2e.sh scripts/gcs-sdk-compat-autoloop internal/services/gcs internal/dashboard go.mod go.sum 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "test(gcs): advance SDK compatibility loop iteration ${iteration}"
}
...
main() {
  ...
  if [[ ! -f "${PROGRESS_FILE}" ]]; then
    printf '# GCS SDK Compatibility Progress\n' > "${PROGRESS_FILE}"
  fi
  ...
  while (( ITERATION <= MAX_ITERATIONS )); do
    ...
    if [[ "${loop_status}" == "DONE" ]]; then
      if run_verify "${DONE_VERIFY_STAGE}" > "${LOOP_DIR}/verify-done.out" 2>&1; then
        log "[DONE] GCS SDK compatibility E2E completed"
        atomic_write_state "$((ITERATION + 1))" "DONE"
        echo "NEXUS_LOOP_STATUS: DONE"
        echo "NEXUS_LOOP_SUMMARY: GCS SDK compatibility E2E completed."
        exit 0
      fi
