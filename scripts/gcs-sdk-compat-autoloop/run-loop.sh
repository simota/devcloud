...
- GCS service implementation is services/gcs.
...
2. Add a temporary first-party SDK smoke program inside the script using first-party Storage SDK.
...
5. Keep scripts/gcs-e2e.sh, VERIFY_STAGE=full bash scripts/gcs-autoloop/verify.sh, and cargo test --workspace green.
...
autocommit_if_requested() {
  ...
  git add README.md docs scripts/gcs-sdk-e2e.sh scripts/gcs-sdk-compat-autoloop 2>/dev/null || true
