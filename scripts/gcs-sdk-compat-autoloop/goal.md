...
2. The E2E path uses the official Go Storage client (`cloud.google.com/go/storage`) or an equivalent first-party Google Cloud Storage SDK client. Prefer Go because this repository is Go-first and already uses `go test`.
...
- `sdk-go`: Go Storage SDK client dependency/import and SDK workflow exist.
