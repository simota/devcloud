...
run_redis_client_probe() {
  local probe="${TMP_DIR}/redis_client_probe.go"
  cat > "${probe}" <<'EOF'
package main
...
func main() {
	...
	if err := client.Set(ctx, "devcloud:e2e:string", "value", 0).Err(); err != nil {
		panic(err)
	}
	got, err := client.Get(ctx, "devcloud:e2e:string").Result()
	if err != nil {
		panic(err)
	}
	if got != "value" {
		panic("GET returned unexpected value")
	}
	if ok, err := client.Expire(ctx, "devcloud:e2e:string", 60*time.Second).Result(); err != nil || !ok {
		panic(fmt.Sprintf("EXPIRE ok=%t err=%v", ok, err))
	}
	if added, err := client.HSet(ctx, "devcloud:e2e:hash", "field", "hash-value").Result(); err != nil || added != 1 {
		panic(fmt.Sprintf("HSET added=%d err=%v", added, err))
	}
	hash, err := client.HGetAll(ctx, "devcloud:e2e:hash").Result()
	if err != nil {
		panic(err)
	}
	if hash["field"] != "hash-value" {
		panic("HGETALL returned unexpected value")
	}
}
EOF
  REDIS_PORT="${REDIS_PORT}" go run "${probe}"
}

run_redis_cli_probe() {
  if ! command -v redis-cli >/dev/null 2>&1; then
    log "[SKIP] redis-cli ping probe (redis-cli binary not in PATH)"
    return
  fi
  local pong
...
log "checking redis-cli PING"
run_redis_cli_probe

log "running go-redis client journey"
run_redis_client_probe
