...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Redshift Query Runner UI Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Redshift query runner UI loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-query-runner-ui
...
echo "[OK] Redshift query runner UI autoloop bootstrap complete"
echo "Run: bash scripts/redshift-query-runner-ui-autoloop/run-loop.sh"
