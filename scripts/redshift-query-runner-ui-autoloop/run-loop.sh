...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-query-runner-ui}"
...
preflight() {
  ...
  if [[ ! -f "${GOAL_FILE}" || ! -f "web/dashboard/src/app/services/redshift/RedshiftDashboard.tsx" ]]; then
    log "[PREFLIGHT:FAIL] Missing goal or Redshift dashboard file"
    exit 1
  fi
  if [[ -f "${LOCK_FILE}" ]]; then
    local pid
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/redshift-query-runner-ui-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-query-runner-ui}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/redshift-query-runner-ui-autoloop/goal.md.
- Add an interactive SQL query runner to web/dashboard/src/app/services/redshift/RedshiftDashboard.tsx.
- Use the existing /api/redshift/query backend endpoint via web/dashboard/src/app/services/redshift/api.ts.
- Preserve existing user changes.
- Do not modify scripts/redshift-query-runner-ui-autoloop/progress.md, state.env, state.env.sha256, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
- Keep UI consistent with existing dashboard components and compact operational layout.
- Do not introduce Playwright or a new frontend framework.
- Do not log or display credentials, DSNs with passwords, authorization headers, SQL bind values, COPY credentials, or object payloads.

Recommended implementation order:
1. Add Redshift query request/result TypeScript types.
2. Add runRedshiftQuery() API wrapper.
3. Add query runner UI with textarea, max rows input, run button, error/success state, command tag, columns, rows, and row count.
4. Refresh catalog/statements after successful query.
5. Add or extend tests/E2E contract checks.
6. Run web/dashboard typecheck/build plus Redshift dashboard and E2E gates.

Required workflow:
1. Inspect current Redshift dashboard UI.
...
Use DONE only when VERIFY_STAGE=full-query-runner-ui bash scripts/redshift-query-runner-ui-autoloop/verify.sh passes completely.
...
maybe_commit() {
  ...
  git commit -m "feat(redshift): add dashboard query runner iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/redshift-query-runner-ui-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: Redshift dashboard query runner UI loop completed with verification evidence.
EOF
      log "[DONE] Redshift query runner UI completed"
      exit 0
    fi
  done
