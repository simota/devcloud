...
preflight() {
  ...
  if [[ ! -f "docs/design-dynamodb-compat.md" ]]; then
    log "[PREFLIGHT:FAIL] Missing docs/design-dynamodb-compat.md"
    exit 1
  fi
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/dynamodb-autoloop/recover.sh"
      exit 1
    fi
  fi
...
Goal:
- Read scripts/dynamodb-autoloop/goal.md and docs/design-dynamodb-compat.md.
- Implement the next smallest safe slice toward completing the DynamoDB-compatible server.
- Preserve existing user changes.
- Do not modify scripts/dynamodb-autoloop/progress.md, state.env, state.env.sha256, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
...
- Never log credentials, Authorization headers, signatures, canonical requests, item payloads, or request bodies.

Current repository context:
- docs/design-dynamodb-compat.md defines the DynamoDB compatibility target.
...
- The current fast gate is: VERIFY_STAGE=foundation bash scripts/dynamodb-autoloop/verify.sh
- The final acceptance gate is: VERIFY_STAGE=full bash scripts/dynamodb-autoloop/verify.sh

Recommended implementation order:
1. Config and daemon wiring for services.dynamodb, auth.dynamodb, and server.dynamodbPort.
2. Minimal DynamoDB JSON 1.0 endpoint with X-Amz-Target dispatch and compatible JSON errors.
3. Table lifecycle: CreateTable, DescribeTable, ListTables, DeleteTable.
4. AttributeValue model and item CRUD: PutItem, GetItem, DeleteItem, UpdateItem basics.
5. Expression engine MVP: ConditionExpression, UpdateExpression, ProjectionExpression, KeyConditionExpression, FilterExpression.
6. Query/Scan pagination and sort ordering.
7. GSI/LSI metadata, projection update, and index Query/Scan.
8. SigV4 strict mode.
9. Streams, TTL, transactions, and PartiQL.
10. Dashboard service registry, /dynamodb page, and /api/dynamodb visibility.
...
Use DONE only when VERIFY_STAGE=full bash scripts/dynamodb-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  git commit -m "feat(dynamodb): advance implementation loop iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/dynamodb-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: DynamoDB-compatible server loop completed with verification evidence.
EOF
      log "[DONE] DynamoDB-compatible server loop completed"
      exit 0
    fi
  done
