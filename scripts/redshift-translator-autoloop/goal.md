...
`services/redshift/translator/translator.rs` の `RedshiftToPostgres` translator が対応する Redshift→PostgreSQL 書き換えルールを、`services/redshift/translator/COMPATIBILITY.md` の **R-only** / **R≠P** チェックリスト項目に従って 1 反復 1 件ずつ追加していく。
...
1. **AC-1 (translator test)**: 反復終了時に `cargo test --workspace` が PASS。
2. **AC-2 (redshift regression)**: `cargo test --workspace` が PASS（既存パッケージへの回帰なし）。
3. **AC-3 (build)**: `cargo build -p devcloud-orchestrator` が成功。
4. **AC-4 (checklist flip)**: 1 反復で `COMPATIBILITY.md` の `- [ ] **R-only** ...` または `- [ ] **R≠P** ...` 1 行が `- [x]` に変わる。P-only 項目は対象外。
5. **AC-5 (test coverage)**: 新ルールが `translator_test.rs` のテストケースで covered（テーブル駆動形式、既存スタイルに準拠）。
6. **AC-6 (commit)**: `AUTOCOMMIT=true` のとき 1 反復 1 commit、メッセージは `feat(redshift/translator): <Redshift 構文名>` 形式。ステージは `services/redshift/translator/` 配下のみ。
