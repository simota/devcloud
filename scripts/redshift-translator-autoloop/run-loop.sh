...
Repository: ${ROOT_DIR}
Checklist: internal/services/redshift/translator/COMPATIBILITY.md
Target line ${ITEM_LINE}: ${ITEM_TEXT}

Tasks:
1. Read internal/services/redshift/translator/translator.go and translator_test.go to understand existing patterns (rewriteRedshiftFunctions, translateCreateTable, rewriteParenFunction, postgresDatePart, etc.).
...
3. Add at least one unit test in translator_test.go in the existing table-driven style covering the new rule.
...
5. Verify locally before finishing:
   - go test ./internal/services/redshift/translator/...   (must PASS)
   - go test ./internal/services/redshift/...              (must PASS, regression check)
   - go build -o /tmp/devcloud-codex-check ./cmd/devcloud  (must succeed)

Constraints (strict):
- Go 1.22 standard library only; no new external dependencies.
- gofmt clean; match existing import grouping.
- Modify ONLY:
  * internal/services/redshift/translator/translator.go
  * internal/services/redshift/translator/translator_test.go
  * internal/services/redshift/translator/COMPATIBILITY.md
...
- If the chosen construct turns out to require a new abstraction layer (parser rewrite, new package, schema changes), STOP after analysis, leave a one-line // TODO(translator): <reason> at the relevant existing site in translator.go but do NOT flip the checkbox; the runner will rollback non-flipping iterations.
...
PROMPT
  ...
  if PRE_ITEM_LINE="${ITEM_LINE}" bash "${VERIFY_SCRIPT}" >> "${RUNNER_LOG}" 2>&1; then
    if [[ "${AUTOCOMMIT}" == "true" ]] && [[ "${DRY_RUN}" != "true" ]]; then
      git add internal/services/redshift/translator/translator.go \
              internal/services/redshift/translator/translator_test.go \
              internal/services/redshift/translator/COMPATIBILITY.md
