...
- SQS dashboard API exists in services/dashboard/server.rs under /api/sqs/*.
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore services/dashboard web/dashboard README.md docs scripts/sqs-e2e.sh scripts/sqs-autoloop/verify.sh scripts/sqs-dashboard-management-autoloop 2>/dev/null || true
