...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/sqs-dashboard-management-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
Goal:
- Read scripts/sqs-dashboard-management-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the SQS dashboard management UI.
- Preserve existing user changes and existing SQS compatibility behavior.
...
Current context:
- SQS React UI exists at web/dashboard/src/app/services/sqs/SQSDashboard.tsx.
- SQS dashboard API exists in internal/dashboard/server.go under /api/sqs/*.
- Existing SQS service supports queue, message, receive lease, DLQ, FIFO, visibility, and purge workflows.

Recommended order:
1. Add typed dashboard API helpers for CreateQueue and SendMessage.
2. Add ReceiveMessage and DeleteMessage workflow with receipt-handle safety.
3. Add ChangeMessageVisibility, safer purge confirmation, and DLQ operational detail.
4. Update dashboard tests/E2E and docs.

Safety:
- Mutations must go through /api/sqs/* or the local SQS protocol path, never direct storage calls.
- Do not persist message bodies, receipt handles, credentials, Authorization headers, SigV4 material, or full request bodies.
- Destructive actions must require explicit confirmation.
- Disabled SQS state must not expose active mutation controls.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/sqs-dashboard-management-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-management-ui bash scripts/sqs-dashboard-management-autoloop/verify.sh
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/dashboard web/dashboard README.md docs scripts/sqs-e2e.sh scripts/sqs-autoloop/verify.sh scripts/sqs-dashboard-management-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(sqs): advance dashboard management loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/sqs-dashboard-management-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: SQS dashboard management UI loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/sqs-dashboard-management-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] SQS dashboard management UI completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
