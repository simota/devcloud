...
assert_queue_send_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'createSQSQueue|sendSQSMessage|CreateQueue|SendMessage|message attributes|MessageGroupId|MessageDeduplicationId|FIFO' web/dashboard/src/app/services/sqs services/dashboard README.md docs &&
    env -u RIPGREP_CONFIG_PATH rg -q "method: 'POST'|/api/sqs/queues|/messages|/send" web/dashboard/src/app/services/sqs/api.ts services/dashboard/server.rs &&
    cargo test --workspace
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_receive_delete_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'receiveSQSMessage|deleteSQSMessage|ReceiveMessage|DeleteMessage|receiptHandle|receipt handle|selected receipt' web/dashboard/src/app/services/sqs services/dashboard README.md docs &&
...
assert_visibility_purge_dlq_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'changeSQSMessageVisibility|ChangeMessageVisibility|visibility timeout|PurgeQueue|purge confirmation|DLQ|dead-letter|redrive' web/dashboard/src/app/services/sqs services/dashboard README.md docs &&
