...
server_line_budget() {
  local lines
  lines="$(wc -l < services/dynamodb/server.rs | tr -d ' ')"
...
focused_file_layout() {
  local count
  count="$(find services/dynamodb -maxdepth 1 -type f -name '*.rs' \
...
package_name_consistent() {
  for file in services/dynamodb/*.rs; do
...
case "${VERIFY_STAGE}" in
  foundation)
    ...
    run_check "rustfmt is clean for DynamoDB package" rustfmt_clean
    ;;
  shape)
