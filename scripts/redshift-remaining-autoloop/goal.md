...
NEXUS_LOOP_SUMMARY: All ACs satisfied: default backend is postgres/managed (config.rs DefaultConfig), actionable diagnostics for missing postgres binaries, MIG-001-009 marked Done in docs/design-redshift-compat.md, and all three verify gates (redshift-autoloop full, redshift-postgres-backend-autoloop full-postgres, redshift-managed-postgres-autoloop full-managed) plus cargo test --workspace pass.
