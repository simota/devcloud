...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Redshift Remaining Tasks Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Redshift remaining-tasks loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-remaining
...
echo "[OK] Redshift remaining-tasks autoloop bootstrap complete"
echo "Run: bash scripts/redshift-remaining-autoloop/run-loop.sh"
