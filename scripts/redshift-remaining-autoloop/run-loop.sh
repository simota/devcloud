...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-remaining}"
...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/redshift-remaining-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-remaining}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/redshift-remaining-autoloop/goal.md and docs/design-redshift-compat.md.
- Implement the next smallest safe slice toward completing remaining Redshift PostgreSQL-backed work.
- Preserve existing user changes.
- Do not modify scripts/redshift-remaining-autoloop/progress.md, state.env, state.env.sha256, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
- Keep backend.kind=memory as an explicit documented fallback.
- Do not remove external DSN mode.
...
1. Make Redshift default backend postgres while preserving explicit memory fallback.
2. Ensure default postgres mode uses managed PostgreSQL without external DSN.
3. Update config generation and tests.
4. Update docs for managed PostgreSQL system-binary strategy and MIG-001..MIG-009 status.
5. Run full Redshift, full-postgres, full-managed, and repository tests.
...
Use DONE only when VERIFY_STAGE=full-remaining bash scripts/redshift-remaining-autoloop/verify.sh passes completely.
...
maybe_commit() {
  ...
  git commit -m "feat(redshift): finish remaining postgres task iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/redshift-remaining-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: Redshift remaining-tasks loop completed with verification evidence.
EOF
      log "[DONE] Redshift remaining tasks completed"
      exit 0
    fi
  done
