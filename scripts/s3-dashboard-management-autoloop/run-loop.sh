...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/s3-dashboard-management-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
Goal:
- Read scripts/s3-dashboard-management-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the S3 dashboard management UI.
- Preserve existing user changes and existing S3 compatibility behavior.
...
Current context:
- S3 React UI exists under web/dashboard/src/app/services/s3/.
- S3 dashboard API exists in internal/dashboard/server.go under /api/s3/*.
...
Recommended order:
1. Add typed dashboard API helpers for CreateBucket, DeleteBucket, PutObject, DeleteObject, and CopyObject.
2. Add guarded bucket/object management controls in the React S3 dashboard.
3. Add upload, copy, download, metadata detail, and disabled-state safety.
4. Add multipart upload visibility/abort and presigned URL affordances where the service already supports them.
5. Update dashboard tests/E2E and docs.

Safety:
- Mutations must go through /api/s3/* or the local S3-compatible endpoint path, never direct storage file writes.
- Do not persist object bodies, credentials, Authorization headers, SigV4 material, presigned secrets, or full request bodies.
- Destructive actions must require explicit confirmation.
- Disabled S3 state must not expose active mutation controls.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/s3-dashboard-management-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-management-ui bash scripts/s3-dashboard-management-autoloop/verify.sh
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/dashboard web/dashboard README.md docs scripts/s3-e2e.sh scripts/s3-autoloop/verify.sh scripts/s3-dashboard-management-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(s3): advance dashboard management loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/s3-dashboard-management-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: S3 dashboard management UI loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/s3-dashboard-management-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] S3 dashboard management UI completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
