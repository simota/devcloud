...
- S3 dashboard API exists in services/dashboard/server.rs under /api/s3/*.
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore services/dashboard web/dashboard README.md docs scripts/s3-e2e.sh scripts/s3-autoloop/verify.sh scripts/s3-dashboard-management-autoloop 2>/dev/null || true
