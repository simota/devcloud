...
assert_bucket_object_management_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'createS3Bucket|deleteS3Bucket|deleteS3Object|CreateBucket|DeleteBucket|DeleteObject|confirmation|disabled' web/dashboard/src/app/services/s3 internal/dashboard README.md docs &&
    env -u RIPGREP_CONFIG_PATH rg -q "method: 'POST'|method: 'PUT'|method: 'DELETE'|/api/s3/buckets" web/dashboard/src/app/services/s3/api.ts internal/dashboard/server.go &&
    go test ./internal/dashboard -run 'TestS3.*Dashboard|TestDashboard.*S3|Test.*S3.*Bucket|Test.*S3.*Object' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_upload_copy_download_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'uploadS3Object|copyS3Object|getS3Object|PutObject|CopyObject|downloadUrl|contentType|metadata|ETag|Last-Modified' web/dashboard/src/app/services/s3 internal/dashboard README.md docs &&
    ...
    go test ./internal/dashboard ./internal/services/s3 -run 'TestS3.*Upload|TestS3.*Copy|TestS3.*Metadata|TestS3.*Dashboard|TestDashboard.*S3' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_multipart_presign_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'multipart|uploadId|abort|presigned|signed URL|presign|X-Amz-Signature' web/dashboard/src/app/services/s3 internal/dashboard README.md docs scripts/s3-e2e.sh scripts/s3-dashboard-management-autoloop 2>/dev/null &&
    env -u RIPGREP_CONFIG_PATH rg -q 'abort|confirmation|disabled|UploadId|PartNumber|presigned' web/dashboard/src/app/services/s3 internal/dashboard/server.go &&
    go test ./internal/dashboard ./internal/services/s3 -run 'TestS3.*Multipart|TestS3.*Presign|Test.*S3.*Dashboard' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}
...
assert_e2e_docs_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'S3 dashboard management|CreateBucket|PutObject|CopyObject|DeleteObject|DeleteBucket|multipart|presigned|metadata' README.md docs scripts/s3-e2e.sh scripts/s3-dashboard-management-autoloop 2>/dev/null &&
    go test ./internal/dashboard ./internal/services/s3 -count=1
}
