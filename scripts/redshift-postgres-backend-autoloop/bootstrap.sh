...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Redshift PostgreSQL Backend Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Redshift PostgreSQL backend migration loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-postgres
...
echo "[OK] Redshift PostgreSQL backend autoloop bootstrap complete"
echo "Run: bash scripts/redshift-postgres-backend-autoloop/run-loop.sh"
