...
Codex-driven autonomous loop for migrating the Redshift-compatible server from the current Go memory executor to a PostgreSQL-backed execution engine.
...
- `full-postgres`: runs all PostgreSQL backend migration gates plus `go test ./...`.
