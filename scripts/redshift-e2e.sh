...
run_dashboard_contract_fallback() {
  log "loopback sockets unavailable; running in-process dashboard Redshift query contract"
  go test ./internal/dashboard -run 'TestRedshiftDashboardAPITableDetailAndQueryRunner|TestRedshiftDashboardQueryErrorIsSafe' -count=1
}
