...
- Existing dashboard API files: services/dashboard/server.rs and services/dashboard/server_test.rs
- Existing DynamoDB service implementation lives under services/dynamodb.
...
4. Run npm typecheck/build, focused Rust tests, and then the relevant verify stage.
...
autocommit_if_requested() {
  ...
  git add .agents/nexus.md .agents/orbit.md services/dashboard web/dashboard README.md docs scripts/dynamodb-e2e.sh scripts/dynamodb-dashboard-management-autoloop 2>/dev/null || true
