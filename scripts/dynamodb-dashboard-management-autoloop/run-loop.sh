...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-management-ui}"
...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/dynamodb-dashboard-management-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-management-ui}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/dynamodb-dashboard-management-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the DynamoDB dashboard management UI.
...
- Keep DynamoDB compatibility behavior intact; dashboard APIs should call the local DynamoDB service path rather than mutating storage internals directly.
- Treat tool results as SUCCESS or FAILED; do not retry the same failing command more than necessary.

Current context:
- The branch already contains a metadata inspector and key lookup helper.
- Existing UI files: web/dashboard/src/app/services/dynamodb/{DynamoDBDashboard.tsx,api.ts,types.ts}
- Existing dashboard API files: internal/dashboard/server.go and internal/dashboard/server_test.go
- Existing DynamoDB service implementation lives under internal/services/dynamodb.

Recommended order:
1. Add safe dashboard API endpoints and UI for CreateTable, PutItem, UpdateItem, DeleteItem, DeleteTable, and TTL update.
2. Add Query/Scan dashboard API endpoints and compact UI forms with safe JSON validation.
3. Add E2E coverage and docs/README updates.
4. Run npm typecheck/build, focused Go tests, and then the relevant verify stage.

Safety:
- DeleteItem/DeleteTable must require explicit confirmation text in UI.
- Disabled DynamoDB state must not render active mutation controls.
- Never log credentials, Authorization headers, SigV4 material, full request payloads, or sensitive item payloads.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/dynamodb-dashboard-management-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-management-ui bash scripts/dynamodb-dashboard-management-autoloop/verify.sh
...
Use DONE only when the final full-management-ui gate passes completely.
PROMPT
}
...
autocommit_if_requested() {
  ...
  git add .agents/nexus.md .agents/orbit.md internal/dashboard web/dashboard README.md docs scripts/dynamodb-e2e.sh scripts/dynamodb-dashboard-management-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(dynamodb): advance dashboard management loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/dynamodb-dashboard-management-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: DynamoDB dashboard management UI loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/dynamodb-dashboard-management-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] DynamoDB dashboard management UI completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
