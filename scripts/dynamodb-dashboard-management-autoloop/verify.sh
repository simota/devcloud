...
assert_current_dashboard_foundation() {
  env -u RIPGREP_CONFIG_PATH rg -q 'getDynamoDBTable|getDynamoDBIndexes|getDynamoDBTTL|getDynamoDBStreams' web/dashboard/src/app/services/dynamodb/api.ts &&
    ...
    go test ./internal/dashboard ./internal/services/dynamodb -run 'TestDynamoDB|Test.*DynamoDB' -count=1
}

assert_operations_ui_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'CreateTable|PutItem|UpdateItem|DeleteItem|DeleteTable|UpdateTimeToLive|confirm|confirmation|delete' web/dashboard/src/app/services/dynamodb internal/dashboard &&
    ...
    go test ./internal/dashboard -run 'TestDynamoDB.*Operation|TestDynamoDB.*Management|TestDynamoDB.*TTL|TestDynamoDB.*Delete' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}

assert_query_scan_ui_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Query|Scan|KeyConditionExpression|FilterExpression|ExpressionAttributeValues|IndexName|ScannedCount|Count' web/dashboard/src/app/services/dynamodb internal/dashboard &&
    ...
    go test ./internal/dashboard -run 'TestDynamoDB.*Query|TestDynamoDB.*Scan' -count=1 &&
    (cd web/dashboard && npm run typecheck && npm run build)
}
...
assert_e2e_docs_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'DynamoDB dashboard API|CreateTable|PutItem|Query|Scan|TTL|streams|indexes' README.md docs scripts &&
    ...
    go test ./internal/dashboard ./internal/services/dynamodb -count=1
}
