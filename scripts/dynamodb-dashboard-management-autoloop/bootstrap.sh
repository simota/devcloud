...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# DynamoDB Dashboard Management Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: DynamoDB dashboard management UI loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-management-ui
...
echo "[OK] DynamoDB dashboard management autoloop bootstrap complete"
echo "Run: bash scripts/dynamodb-dashboard-management-autoloop/run-loop.sh"
