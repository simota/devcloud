...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/s3-test-coverage-autoloop/recover.sh"
      exit 1
    fi
  fi
  log "[PREFLIGHT] OK"
...
Goal:
- Read scripts/s3-test-coverage-autoloop/goal.md.
- Implement the next smallest safe slice toward strengthening S3 test coverage.
- Preserve existing user changes and existing S3 compatibility/dashboard behavior.
...
Current context:
- S3 service tests live in internal/services/s3/server_test.go.
- S3 dashboard API tests live in internal/dashboard/server_test.go.
- S3 E2E coverage lives in scripts/s3-e2e.sh.
- Existing S3 service supports bucket/object CRUD, metadata, range reads, CopyObject, multipart uploads, and SigV4/presign verification.

Recommended order:
1. Add S3 service edge tests for invalid bucket/key behavior, metadata/content headers, range errors, copy preconditions, multipart abort/list, and presigned URL validation.
2. Add dashboard S3 API tests for validation, disabled service responses, metadata propagation, mutation safety, and destructive confirmations.
3. Extend scripts/s3-e2e.sh or docs to cover S3 management workflows and test intent.
4. Raise S3 service coverage to at least 72.0% and dashboard package coverage to at least 68.5%.
...
- Do not log object bodies, credentials, Authorization headers, SigV4 material, presigned secrets, or full request bodies.
- Do not add arbitrary sleeps or external AWS dependencies.
- Isolate temporary state per test.

Verification:
- Fast gate: VERIFY_STAGE=foundation bash scripts/s3-test-coverage-autoloop/verify.sh
- Final gate: VERIFY_STAGE=full-test-coverage bash scripts/s3-test-coverage-autoloop/verify.sh
...
autocommit_if_requested() {
  ...
  git add .agents/orbit.md .gitignore internal/services/s3 internal/dashboard README.md docs scripts/s3-e2e.sh scripts/s3-autoloop/verify.sh scripts/s3-dashboard-management-autoloop/verify.sh scripts/s3-test-coverage-autoloop 2>/dev/null || true
  if git diff --cached --quiet; then
    return 0
  fi
  git commit -m "feat(s3): advance test coverage loop iteration ${iteration}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/s3-test-coverage-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: S3 test coverage loop completed with verification evidence.
EOF
}
...
run_iteration() {
  ...
  set +e
  VERIFY_STAGE="${verify_stage}" bash scripts/s3-test-coverage-autoloop/verify.sh > "${LOOP_DIR}/verify-${ITERATION}.out" 2>&1
  local verify_exit=$?
  set -e
  ...
  if [[ "${verify_exit}" -eq 0 ]]; then
    ...
    if [[ "${status}" == "DONE" ]]; then
      mark_done "${ITERATION}"
      log "[DONE] S3 test coverage completed"
      exit 0
    fi
  else
    CIRCUIT_FAIL_COUNT=$((CIRCUIT_FAIL_COUNT + 1))
