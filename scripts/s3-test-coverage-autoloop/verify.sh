...
assert_service_edge_tests() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Invalid|Error|Range|Metadata|Copy|Multipart|Presign|Delete|NotFound|Precondition|Header' internal/services/s3/server_test.go &&
    go test ./internal/services/s3 -run 'Test.*(Invalid|Error|Range|Metadata|Copy|Multipart|Presign|Delete|NotFound|Precondition|Header)' -count=1
}

assert_dashboard_api_tests() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Test.*S3.*(Dashboard|API|Disabled|Invalid|Upload|Copy|Delete|Multipart|Metadata|Confirmation)' internal/dashboard/server_test.go &&
    env -u RIPGREP_CONFIG_PATH rg -q '/api/s3|disabled|invalid|metadata|multipart|copy|delete|confirmation' internal/dashboard/server_test.go &&
    go test ./internal/dashboard -run 'Test.*S3.*(Dashboard|API|Disabled|Invalid|Upload|Copy|Delete|Multipart|Metadata|Confirmation)' -count=1
}
...
assert_e2e_docs_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'S3 test coverage|bucket create|bucket delete|object upload|object delete|object copy|metadata|multipart|presigned|CreateBucket|PutObject|CopyObject|DeleteObject|AbortMultipart' README.md docs scripts/s3-e2e.sh scripts/s3-test-coverage-autoloop 2>/dev/null &&
    go test ./internal/services/s3 ./internal/dashboard -count=1
}
...
assert_coverage_thresholds() {
  assert_min_coverage ./internal/services/s3 72.0 &&
    assert_min_coverage ./internal/dashboard 68.5
}
