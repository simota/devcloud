...
2. `internal/services/s3/server_test.go` gains targeted tests for S3 edge/error behavior across bucket, object, copy, metadata, range, multipart, and presigned URL flows.
3. `internal/dashboard/server_test.go` gains targeted tests for `/api/s3/*` dashboard API validation, disabled-state behavior, mutation safety, metadata propagation, and destructive confirmation behavior where applicable.
...
- Keep S3 tests scoped to `internal/services/s3`, `internal/dashboard`, `scripts/s3-e2e.sh`, README/docs, and this loop directory.
