...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Redshift Managed PostgreSQL Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Redshift managed PostgreSQL mode loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-managed
...
echo "[OK] Redshift managed PostgreSQL autoloop bootstrap complete"
echo "Run: bash scripts/redshift-managed-postgres-autoloop/run-loop.sh"
