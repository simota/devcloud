...
assert_managed_config_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Backend.*Managed|services\\.redshift\\.backend\\.managed|backend\\.managed|managed' orchestrator services/redshift &&
    cargo test --workspace
}

assert_managed_lifecycle_contract() {
  ! env -u RIPGREP_CONFIG_PATH rg -q 'managed backend is not implemented yet' orchestrator services/redshift &&
    env -u RIPGREP_CONFIG_PATH rg -q 'initdb|pg_ctl|postgres.*-D|ManagedPostgres|managed.*postgres|PostgresProcess|Shutdown|Close' orchestrator services/redshift &&
    cargo test --workspace
}
