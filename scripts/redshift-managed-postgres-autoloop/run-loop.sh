...
DONE_VERIFY_STAGE="${DONE_VERIFY_STAGE:-full-managed}"
...
preflight() {
  ...
  if [[ -f "${STATE_FILE}" && -f "${STATE_FILE}.sha256" ]]; then
    ...
    if [[ "${expected}" != "${actual}" ]]; then
      log "[PREFLIGHT:FAIL] state.env checksum mismatch; run scripts/redshift-managed-postgres-autoloop/recover.sh"
      exit 1
    fi
  fi
...
load_state() {
  ...
  DONE_VERIFY_STAGE="${REQUESTED_DONE_VERIFY_STAGE:-${DONE_VERIFY_STAGE:-full-managed}}"
  CIRCUIT_STATE="${CIRCUIT_STATE:-CLOSED}"
  CIRCUIT_FAIL_COUNT="${CIRCUIT_FAIL_COUNT:-0}"
}
...
Goal:
- Read scripts/redshift-managed-postgres-autoloop/goal.md and docs/design-redshift-compat.md.
- Implement the next smallest safe slice toward Redshift managed PostgreSQL mode.
- Preserve existing user changes.
- Do not modify scripts/redshift-managed-postgres-autoloop/progress.md, state.env, state.env.sha256, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
- Keep external DSN mode and backend.kind=memory behavior compatible.
- Prefer focused tests for config, lifecycle, startup failure, shutdown cleanup, redaction, and E2E script behavior.
- Never log passwords, DSNs with credentials, authorization headers, SQL bind values, COPY credentials, object payloads, or statement results unless explicitly redacted.

Current repository context:
- external PostgreSQL backend mode exists.
- managed PostgreSQL currently has a not-implemented guard in orchestrator.
- scripts/redshift-e2e.sh can already accept REDSHIFT_BACKEND_KIND, REDSHIFT_BACKEND_MODE, REDSHIFT_BACKEND_EXTERNAL_DSN, and REDSHIFT_BACKEND_MANAGED.

Recommended implementation order:
1. Introduce a small managed PostgreSQL lifecycle abstraction with injectable command discovery for tests.
2. Discover system PostgreSQL binaries from PATH and return actionable errors when unavailable.
3. Initialize the managed data directory idempotently.
4. Start PostgreSQL on a local port and synthesize a redacted-safe DSN for the existing backend/postgres package.
5. Stop the managed process on daemon shutdown.
6. Add scripts/redshift-managed-postgres-e2e.sh that runs the existing Redshift E2E with managed backend mode.
7. Update docs only when behavior or commands change.
...
Use DONE only when VERIFY_STAGE=full-managed bash scripts/redshift-managed-postgres-autoloop/verify.sh passes completely.
...
maybe_commit() {
  ...
  git commit -m "feat(redshift): add managed postgres iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/redshift-managed-postgres-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: Redshift managed PostgreSQL mode loop completed with verification evidence.
EOF
      log "[DONE] Redshift managed PostgreSQL mode completed"
      exit 0
    fi
  done
