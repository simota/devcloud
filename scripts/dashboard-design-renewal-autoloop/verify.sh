...
dashboard_registry_tests() {
  go test ./internal/dashboard ./internal/app -run 'TestDashboardServices|TestDaemonDoesNotExposeS3DashboardAPIWhenS3Disabled' -count=1
}
...
case "${VERIFY_STAGE}" in
  foundation)
    ...
    run_check "Go foundation passes" go_foundation
