...
4. All service pages are served under `/dashboard/<svc>` from the shared React shell. Compatibility `/mail`, `/s3`, `/gcs`, `/dynamodb`, `/bigquery` return 301 redirects to the matching `/dashboard/<svc>` so existing bookmarks keep working. `/`, `/api/messages/*`, and `/api/<svc>/*` keep their current contracts unchanged.
...
1. `foundation`: keep Rust tests, existing dashboard APIs, and current static routes healthy.
...
4. `embed-assets`: add compatibility embedded asset serving for built React dashboard assets without intercepting `/api/*`.
...
- Keep production runtime as a Rust binary; Node/Vite is development/build-time only.
