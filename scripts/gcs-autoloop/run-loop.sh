...
preflight() {
  ...
  if [[ ! -f "docs/design-gcs-compat.md" ]]; then
    log "[PREFLIGHT:FAIL] Missing docs/design-gcs-compat.md"
    exit 1
  fi
...
Goal:
- Read scripts/gcs-autoloop/goal.md and docs/design-gcs-compat.md.
- Implement the next smallest safe slice toward completing the GCS-compatible server.
- Preserve existing user changes.
- Do not modify scripts/gcs-autoloop/progress.md, state.env, runner.log, runner.jsonl, iteration outputs, or done.md; the runner owns loop state.
- Use Rust standard library unless a dependency is unavoidable.
- Keep generated runtime data under .devcloud/.
- Preserve existing Mail, S3, and dashboard behavior.
- Prefer tests for each behavior you add.

Current repository context:
- docs/design-gcs-compat.md defines the GCS compatibility target.
- docs/design-s3-compat.md and services/s3 show the closest existing object storage implementation.
- docs/design-dashboard-shell.md defines the shared dashboard shell direction.
- The current fast gate is: VERIFY_STAGE=foundation bash scripts/gcs-autoloop/verify.sh
- The final acceptance gate is: VERIFY_STAGE=full bash scripts/gcs-autoloop/verify.sh

Recommended implementation order:
1. Config and daemon wiring for services.gcs and server.gcsPort.
2. Minimal GCS JSON API server with buckets insert/get/list/delete.
3. Object media upload/download/list/delete/copy.
4. Metadata, generation/metageneration, range, and preconditions.
5. Resumable upload.
6. Dashboard service registry and /api/gcs visibility.
...
Use DONE only when VERIFY_STAGE=full bash scripts/gcs-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  git commit -m "feat(gcs): advance implementation loop iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/gcs-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: GCS-compatible server loop completed with verification evidence.
EOF
      log "[DONE] GCS-compatible server loop completed"
      exit 0
    fi
  done
