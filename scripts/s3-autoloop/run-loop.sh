...
Goal:
- Read scripts/s3-autoloop/goal.md.
- Implement the next smallest safe slice toward completing the S3-compatible server MVP.
- Preserve existing user changes.
- Do not modify scripts/s3-autoloop/progress.md, state.env, runner.log, runner.jsonl, or done.md; the runner owns loop state.
...
- docs/design-s3-compat.md defines the S3 compatibility target.
- docs/design-s3-ui.md and mock/s3 define the dashboard reference.
- Implementation should follow the phases in scripts/s3-autoloop/goal.md.
- Preserve existing Mail behavior while adding S3.
- The current fast gate is: VERIFY_STAGE=foundation bash scripts/s3-autoloop/verify.sh
- The final acceptance gate is: VERIFY_STAGE=full bash scripts/s3-autoloop/verify.sh
...
Use DONE only when VERIFY_STAGE=full bash scripts/s3-autoloop/verify.sh passes completely.
PROMPT
}
...
maybe_commit() {
  ...
  if [[ -z "$(git status --short -- . ':!scripts/s3-autoloop/runner.log' ':!scripts/s3-autoloop/runner.jsonl' ':!scripts/s3-autoloop/state.env' ':!scripts/s3-autoloop/progress.md')" ]]; then
    return 0
  fi
  git add . ':!scripts/s3-autoloop/runner.log' ':!scripts/s3-autoloop/runner.jsonl' ':!scripts/s3-autoloop/state.env' ':!scripts/s3-autoloop/progress.md'
  git commit -m "feat(s3): advance implementation loop iteration ${iteration}" -m "Loop status: ${status}"
}
...
- Verification: VERIFY_STAGE=${DONE_VERIFY_STAGE} bash scripts/s3-autoloop/verify.sh passed
- Rollback: revert the implementation commits or restore from git history.

NEXUS_LOOP_STATUS: DONE
NEXUS_LOOP_SUMMARY: S3-compatible server loop completed with verification evidence.
EOF
      log "[DONE] S3-compatible server loop completed"
      exit 0
    fi
  done
