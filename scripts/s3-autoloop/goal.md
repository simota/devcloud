...
The project needs a local S3-compatible server that AWS CLI, AWS SDKs, and common S3 clients can use with only endpoint override configuration. This loop should turn the current Mail-focused emulator into a multi-service local cloud emulator while preserving the documented Go-served static dashboard direction.
...
6. `dashboard-static`: add Go-served S3 dashboard/API following the Object Explorer design.
