...
2. The E2E path uses the official first-party BigQuery client (`first-party BigQuery SDK`) or an equivalent first-party Google BigQuery SDK client. Prefer the first-party SDK path because this repository is Rust-first and already uses `cargo test`.
...
- `sdk-client`: compatibility BigQuery SDK client dependency/import and SDK workflow exist.
