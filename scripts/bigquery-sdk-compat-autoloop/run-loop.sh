...
- BigQuery service implementation is internal/services/bigquery.
...
2. Add a temporary Go SDK smoke program inside the script using cloud.google.com/go/bigquery.
...
5. Keep scripts/bigquery-e2e.sh, VERIFY_STAGE=full bash scripts/bigquery-autoloop/verify.sh, and go test ./... green.
...
autocommit_if_requested() {
  ...
  git add README.md docs scripts/bigquery-sdk-e2e.sh scripts/bigquery-sdk-compat-autoloop internal/services/bigquery internal/dashboard scripts/bigquery-e2e.sh go.mod go.sum 2>/dev/null || true
