...
assert_loop_contract() {
  bash -n scripts/bigquery-sdk-compat-autoloop/bootstrap.sh &&
    ...
    env -u RIPGREP_CONFIG_PATH rg -q 'sdk-go|sdk-e2e|compat-docs|full-sdk-compat|cloud.google.com/go/bigquery|NEXUS_LOOP_STATUS: READY' scripts/bigquery-sdk-compat-autoloop/goal.md &&
...
assert_go_sdk_contract() {
  [[ -f scripts/bigquery-sdk-e2e.sh ]] &&
    bash -n scripts/bigquery-sdk-e2e.sh &&
    env -u RIPGREP_CONFIG_PATH rg -q 'cloud.google.com/go/bigquery|bigquery.NewClient|option.WithEndpoint|option.WithoutAuthentication' scripts/bigquery-sdk-e2e.sh internal docs README.md
}

assert_sdk_workflow_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'Dataset|Table|Inserter|Iterator|Query|Job|Read|Delete|Create' scripts/bigquery-sdk-e2e.sh &&
    env -u RIPGREP_CONFIG_PATH rg -q 'find_free_port|mktemp|devcloud.*up|trap cleanup|E2E_BIGQUERY_PORT|E2E_DASHBOARD_PORT' scripts/bigquery-sdk-e2e.sh
}
...
assert_docs_contract() {
  env -u RIPGREP_CONFIG_PATH rg -q 'bigquery-sdk-e2e|BigQuery SDK|Google BigQuery SDK|cloud.google.com/go/bigquery' README.md docs scripts/bigquery-sdk-compat-autoloop
}
...
run_sdk_go_checks() {
  run_foundation_checks
  run_check "Go BigQuery SDK contract exists" assert_go_sdk_contract
  run_check "SDK workflow contract exists" assert_sdk_workflow_contract
}

run_sdk_e2e_checks() {
  run_sdk_go_checks
  run_check "BigQuery SDK E2E script passes" assert_sdk_e2e_runs
}
