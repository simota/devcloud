...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Pub/Sub Full Compatibility Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Pub/Sub remaining compatibility loop initialized."
  } > "${tmp_progress}"
...
echo "[OK] Pub/Sub full compatibility autoloop bootstrap complete"
echo "Run: bash scripts/pubsub-full-compat-autoloop/run-loop.sh"
