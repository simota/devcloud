...
9. Google Pub/Sub first-party client smoke covers create topic, create subscription, publish, receive, ack, and cleanup using `PUBSUB_EMULATOR_HOST`.
10. Existing Mail, S3, GCS, DynamoDB, BigQuery, SQS, dashboard, and Pub/Sub MVP behavior remains compatible; `cargo test --workspace` passes.
...
- Use official `first-party Pub/Sub SDK generated types` and gRPC server interfaces.
