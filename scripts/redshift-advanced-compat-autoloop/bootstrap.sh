...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Redshift Advanced Compatibility Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Redshift advanced compatibility loop initialized."
  } > "${tmp_progress}"
...
DONE_VERIFY_STAGE=full-advanced
...
echo "[OK] Redshift advanced compatibility autoloop bootstrap complete"
echo "Run: bash scripts/redshift-advanced-compat-autoloop/run-loop.sh"
