...
if [[ ! -f "${PROGRESS_FILE}" ]]; then
  ...
  {
    echo "# Pub/Sub Autoloop Progress"
    echo ""
    echo "Created at: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo ""
    echo "NEXUS_LOOP_STATUS: READY"
    echo "NEXUS_LOOP_SUMMARY: Pub/Sub implementation loop initialized."
  } > "${tmp_progress}"
...
bash -n "${ROOT_DIR}/scripts/pubsub-e2e.sh"

echo "[OK] Pub/Sub autoloop bootstrap complete"
echo "Run: bash scripts/pubsub-autoloop/run-loop.sh"
