...
assert_redshift_config_shape() {
  env -u RIPGREP_CONFIG_PATH rg -q 'redshiftPort|redshiftAPIPort|services\\.redshift|auth\\.redshift|Redshift|redshift' internal/app internal/dashboard cmd &&
    go test ./internal/app -run 'Test.*Redshift|TestDefaultConfig|TestInitWorkspace|TestLoadConfig' -count=1
}
...
assert_postgres_backend_shape() {
  test -f internal/services/redshift/backend/backend.go &&
    ...
    env -u RIPGREP_CONFIG_PATH rg -q 'type SQLBackend|backend\\.kind|postgres' internal/services/redshift internal/app
}

assert_translator_shape() {
  test -d internal/services/redshift/translator &&
    env -u RIPGREP_CONFIG_PATH rg -q 'type RedshiftTranslator|Translate\(|GETDATE|SYSDATE|NVL|DECODE|DATEADD|DATEDIFF|LISTAGG|DISTKEY|SORTKEY|ENCODE' internal/services/redshift/translator internal/services/redshift
}
