//! legacy-compatible JSON encoding for the Pub/Sub REST service.
//!
...
//! newline). Both apply legacy `encoding/json` HTML escaping (`<`, `>`, `&` and
//! U+2028/U+2029 inside string values). serde leaves those raw, so a post-pass
//! byte rewrite over the already-valid JSON reproduces legacy exactly. Struct fields
//! keep declaration order; `BTreeMap` keys (and `serde_json::Value` objects, with
//! `preserve_order` off) come out sorted, matching legacy map marshaling.

use serde::Serialize;

/// Rewrites `<`, `>`, `&` and the U+2028/U+2029 line separators to their `\uXXXX`
/// escapes, matching legacy `encoding/json` HTML escaping.
