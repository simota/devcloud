...
//!
//! Mirrors `internal/services/pubsub/{routes,...}.rs`: a single handler routes by
...
//! with legacy `writeJSON` headers. A hand-rolled HTTP/1.1 reader/writer on plain
...
pub fn route(server: &mut Server, req: &Request) -> RestResponse {
    ...
    if path == "/healthz" || path == "/readyz" {
        ...
        return RestResponse {
            status: 200,
            body: crate::wire_json::to_vec(&server.health_body()),
            allow: None,
            www_authenticate: false,
        };
    }

    // Read-only introspection API (dashboard browse). GET-only; matched before
    // the `/v1/` provider routing so it can never collide with the protocol.
    if crate::introspect::is_introspect_path(path) {
        return crate::introspect::handle_introspect(server, method, path);
    }
