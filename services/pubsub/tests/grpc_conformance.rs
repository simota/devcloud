...
async fn dial(cfg: EngineConfig) -> Engine {
    ...
    let adapter = PubSubGrpc::new(state.clone());
...
async fn regression_rest_and_grpc_share_resources_deliveries_and_acknowledgements() {
    ...
    assert_eq!(
        received["receivedMessages"][0]["message"]["data"],
        "ZnJvbSBncnBj"
    );
    let ack_id = received["receivedMessages"][0]["ackId"]
        .as_str()
        .unwrap()
        .to_string();
    e.subscriber
    ...
    rest(
        &e,
        "PUT",
        &format!("/v1/{inverse_sub}"),
        serde_json::json!({"topic": inverse_topic}),
    );
    assert_eq!(
        e.subscriber
            .get_subscription(GetSubscriptionRequest {
                subscription: inverse_sub
            })
            .await
            .unwrap()
            .into_inner()
