...
//! legacy server (`writeJSON` / `writeJSONError`).
...
fn send_receive_delete_roundtrip() {
    ...
    // SendMessage → {MessageId, MD5OfMessageBody} (md5 of "hello" from legacy oracle).
    let (status, _, body) = call(
        &mut s,
        "SendMessage",
...
fn send_message_to_missing_queue_errors() {
    ...
    assert_eq!(status, 400);
    assert_eq!(et.as_deref(), Some("QueueDoesNotExist"));
    // Error body shape matches legacy writeJSONError.
    assert_eq!(
        body,
...
fn lowercase_tags_field_accepted_on_create() {
    // legacy accepts both `Tags` and lowercase `tags` on CreateQueue.
    let mut s = Server::new(cfg());
    call(
        &mut s,
        "CreateQueue",
