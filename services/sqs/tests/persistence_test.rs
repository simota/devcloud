...
//!
//! `state_oracle.json` is the literal output of the legacy `Server.persistLocked`
...
//! legacy bytes exactly — proving field names, ordering, omitempty, and the
...
fn state_json_round_trips_byte_for_byte() {
    ...
    assert_eq!(
        ...
        ORACLE,
        "re-serialized state.json must match the legacy bytes exactly"
    );
}
...
fn parsed_oracle_has_expected_shape() {
    ...
    // Unset message timestamps round-trip as the legacy zero time.
