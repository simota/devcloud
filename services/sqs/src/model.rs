...
//! `internal/services/sqs/{types,persistence}.rs`.
//!
//! Serialization is pinned to legacy `encoding/json`:
//!   * `messageState`, `deduplicationState`, `moveTaskState` have NO legacy json
//!     tags, so they serialize with their exact legacy field names (PascalCase) and
//!     every field is always present.
//!   * `time.Time` fields serialize as RFC 3339; the legacy zero time is the literal
//!     `0001-01-01T00:00:00Z`, which we model as a `String` default.
//!   * `messageAttributeValue` keeps its legacy json tags (omitempty on the value
//!     fields).
...
/// legacy zero `time.Time` in RFC 3339 — emitted for unset message timestamps.
...
/// Mirrors `messageAttributeValue` (keeps legacy json tags + omitempty).
...
/// Mirrors `messageState`. No legacy json tags → exact field names, all always
...
/// Mirrors `deduplicationState`. No legacy json tags → exact field names.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// Mirrors `moveTaskState`. No legacy json tags → exact field names.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
