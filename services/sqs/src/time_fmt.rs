//! RFC 3339 UTC formatting/parsing matching legacy `time.Time` JSON wire form,
...
/// Parses an RFC 3339 UTC timestamp to UNIX seconds. The legacy zero time
/// (`0001-01-01T00:00:00Z`) and unparseable input both yield the legacy zero
...
/// chronological comparison and arithmetic at full precision. The legacy zero time
...
/// Mirrors legacy `time.Time.UnixMilli()` for an RFC 3339 timestamp.
...
/// `true` when the RFC 3339 timestamp is the legacy zero time (or empty).
