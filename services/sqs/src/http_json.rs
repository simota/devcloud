...
//! of `routes.rs` + the per-operation handlers.
//!
...
impl Server {
    ...
    fn json_create_queue(&mut self, req: &Value) -> JsonOutcome {
        ...
        // legacy accepts both `Tags` and lowercase `tags`.
...
/// Reads an optional integer (mirrors legacy `*int` request fields). Absent →
...
/// Mirrors the JSON shape of legacy `receivedMessage` (omitempty fields dropped).
