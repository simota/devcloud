...
//!
//! Field declaration order and `omitempty` semantics mirror the legacy structs in
//! `internal/services/dynamodb/types.rs`. serde emits struct fields in
//! declaration order and `BTreeMap` keys sorted, matching legacy `encoding/json`;
//! the `skip_serializing_if` predicates here reproduce legacy `omitempty`. Generic
//! attribute values use `serde_json::Value` (sorted-key object), matching legacy
//! `map[string]any`.
...
/// `{"M": {...}}`. legacy models this as `map[string]any`.
pub type AttributeValue = Value;

/// An item: attribute name -> attribute value. legacy: `map[string]attributeValue`,
...
/// A resource tag (`Key`/`Value`), mirroring legacy `tag`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct Tag {
    #[serde(rename = "Key")]
    pub key: String,
...
/// A `ListBackups` summary entry, mirroring legacy `backupSummary` field order.
...
/// `omitempty` mirror legacy `streamRecordImage`. `New`/`OldImage` are dropped when
