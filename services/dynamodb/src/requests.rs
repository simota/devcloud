...
//!
//! Mirrors the request structs in `internal/services/dynamodb/types.rs`. Fields
//! use `#[serde(default)]` so absent keys decode to zero values (matching legacy
...
pub struct QueryRequest {
    ...
    #[serde(rename = "Limit")]
    pub limit: i64,
    // legacy uses `*bool` (absent => default ascending); model it as Option.
