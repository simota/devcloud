...
//! loopback address the legacy daemon would have used, reading the same storage dir
//! and writing a byte-compatible `state.json`.
//!
//! Configuration comes entirely from environment variables (set by the legacy daemon
//! seam in `internal/app/dynamodb_rust.rs`):
//!
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        let shutdown = async {
            // Graceful shutdown on SIGTERM (sent by the legacy daemon) or Ctrl-C.
            #[cfg(unix)]
            {
