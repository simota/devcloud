//! Decimal number handling matching legacy `math/big.Rat` usage in the DynamoDB
//! service (validation, comparison, and the `ADD`/arithmetic update operators).
//!
//! DynamoDB `N` values are decimal strings. legacy parses them with
...
//! exponent (the realistic subset DynamoDB clients send). legacy `big.Rat` also
...
impl Decimal {
    ...
    /// `big.Rat.FloatString(precision)` then legacy trailing `0`/`.` trimming, as
    ...
    fn float_string_trimmed(&self, precision: u32) -> String {
        ...
        // Trim trailing zeros, then a dangling dot (legacy TrimRight chain).
        if out.contains('.') {
...
/// `addNumberStrings`: parses both, adds, and formats the way legacy does (integer →
...
/// `negateNumberAttribute`: negates and formats like legacy (integer →
