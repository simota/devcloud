//! `state.json` persistence — byte-compatible with the legacy service.
//!
//! Mirrors `persistedState` / `persistedTable` in
//! `internal/services/dynamodb/persistence.rs`. The legacy service writes
...
//! fields all use legacy `omitempty` (an empty map/slice/string/nil pointer is
//! dropped). `to_state_json` routes through `wire_json::to_vec`.
...
mod tests {
    ...
    fn basic_state_round_trips_byte_for_byte() {
        // Golden oracle captured from the legacy service (CreateTable + PutItem with
        ...
        assert_eq!(
            encoded,
            fixture.to_vec(),
            "re-encoded state.json must match the legacy oracle byte-for-byte\n--- got ---\n{}\n--- want ---\n{}",
