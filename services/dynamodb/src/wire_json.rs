//! legacy-compatible JSON encoding.
//!
//! The legacy DynamoDB service encodes JSON with `encoding/json`, which differs from
//! serde_json defaults in ways we must reproduce byte-for-byte:
//!
//!  1. **HTML escaping** — legacy escapes `<`, `>`, `&` (and the U+2028 / U+2029 line
...
//!  3. **Key ordering** — legacy sorts *map* keys but keeps *struct* fields in
...
//!     So plain [`to_vec`] matches legacy for both `state.json` (struct-ordered top
...
/// escapes, matching legacy `encoding/json` HTML escaping. Operates on valid JSON:
...
/// Compact JSON, legacy HTML escaping, trailing `\n`. Struct fields keep declaration
...
/// Like [`to_vec`] but **without** the trailing newline — matches legacy
/// `json.Marshal` (used for the internal item-key string and for sizing items,
/// where the encoded bytes must match legacy `json.Marshal(...)` exactly).
