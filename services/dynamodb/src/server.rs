...
//!
//! Mirrors `internal/services/dynamodb/{server,table_handlers,persistence}.rs`.
...
//! built from the typed `model` structs so the success bodies match legacy
...
use crate::{time_util, wire_json};

/// The encoded legacy-wire body for a successful operation (field order preserved,
...
/// In-memory state for a single table. Mirrors legacy `tableState`.
#[derive(Clone, Debug)]
...
impl Server {
    ...
    pub fn update_table(&mut self, request: &UpdateTableRequest) -> OpResult {
        ...
        // committing — mirroring legacy rollback-on-error behavior.
    ...
    pub fn put_item(&mut self, request: &PutItemRequest) -> OpResult {
        ...
        Ok(crate::wire_json::to_vec(&Value::Object(response)))
    }
    ...
    pub fn transact_write_items(&mut self, request: &TransactWriteItemsRequest) -> OpResult {
        ...
        self.apply_planned_writes(&plan)?;
        Ok(crate::wire_json::to_vec(&Value::Object(Map::new())))
    }
    ...
    pub fn list_tags_of_resource(&self, request: &ListTagsOfResourceRequest) -> OpResult {
        ...
        // BTreeMap iteration is already key-sorted, matching legacy explicit sort.
    ...
    fn enable_stream_description(
        ...
        // legacy: `description.TableArn + "/stream/" + label`, with a fallback for an
...
/// `addConsumedCapacity`: `CapacityUnits` is the JSON integer `1` (legacy writes
...
/// The cancelled-transaction error legacy returns for any failed condition inside a
