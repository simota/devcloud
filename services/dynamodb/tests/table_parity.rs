...
//! oracles captured from the legacy service (`internal/services/dynamodb`).
...
use devcloud_dynamodb::wire_json;
...
/// Builds the same `Orders` table the legacy oracle test created.
...
#[test]
fn create_table_response_matches_legacy_oracle() {
    let dir = tempdir();
    ...
    assert_eq!(
        ...
        "CreateTable response body must match the legacy oracle byte-for-byte"
    );
}
...
fn create_table_persists_byte_compatible_state() {
    ...
    assert_eq!(
        ...
        String::from_utf8_lossy(&want),
        "state.json must match the legacy oracle byte-for-byte"
    );
}
...
fn duplicate_create_table_is_resource_in_use() {
    ...
    let body = wire_json::to_vec(&err.body());
    assert_eq!(
...
#[test]
fn create_table_validation_messages_match_legacy() {
    let dir = tempdir();
    let mut s = server_with_storage(&dir);
