...
//! RestoreTableFromBackup) against golden oracles captured from the legacy service.
...
/// Reproduces the full legacy oracle scenario (TTL + continuous backups + backups +
/// restore + delete) and checks each response plus the final state.json.
#[test]
fn ttl_backups_scenario_matches_legacy_oracle() {
    let dir = tempdir();
