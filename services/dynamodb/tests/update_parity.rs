...
//! if_not_exists, list_append) against golden oracles captured from the legacy
//! service.
...
/// Reproduces the legacy oracle scenario end-to-end and checks every response body
/// plus the final state.json, all byte-for-byte.
#[test]
fn update_scenario_matches_legacy_oracle() {
    let dir = tempdir();
