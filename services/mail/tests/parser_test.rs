//! 1:1 parity port of `internal/services/mail/parser_test.rs`.
...
// --- Differential-parity edge cases, pinned to the legacy golden oracle ---
...
fn parity_header_without_blank_terminator_no_newline() {
    // legacy golden oracle: a header block with no blank-line terminator and no
...
fn parity_header_without_blank_terminator_trailing_crlf() {
    // legacy golden oracle: same as above with a trailing CRLF — parseErr="".
    let env = Envelope {
        from: "ENV".to_string(),
        ..Default::default()
    };
...
fn parity_multipart_part_with_trailing_blank_line() {
    // legacy golden oracle: the part body keeps its own trailing CRLF — only the
...
fn parity_multipart_quoted_boundary_with_semicolon() {
    // legacy golden oracle: mime.ParseMediaType is quote-aware, so `boundary="a;b"`
