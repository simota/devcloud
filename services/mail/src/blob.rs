//! Mirrors `internal/storage/blob/store.rs`.
//!
...
//! a temp file + atomic rename, matching the legacy implementation byte-for-byte
//! (same id, same shard path), so a Rust writer and a legacy reader interoperate.
...
/// Blob identifier — the lowercase hex SHA-256 of the content. Mirrors legacy
/// `blob.ID`.
pub type BlobId = String;

/// Content-addressed blob store backed by the filesystem. Mirrors legacy
...
/// legacy `os.CreateTemp` + `os.Rename` durability pattern.
