//! Part 2 parity: the `FileBucketStore` bucket-CRUD plane reproduces the legacy
...
//! `foundation_parity.rs`; here we verify structure and behavior against the legacy
...
/// Sorted set of paths under `root`, dirs suffixed with `/` (matches the legacy
/// oracle's `relTree`).
...
fn bucket_crud_matches_oracle() {
    ...
    // Object-path encodings (relative to root), pinned to the legacy oracle.
    let cases = [
