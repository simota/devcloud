//! UTC RFC3339Nano formatting, matching legacy
...
//! fraction is trailing-zero-trimmed (legacy drops all trailing zero digits).

use std::time::{SystemTime, UNIX_EPOCH};

/// legacy `time.Time` zero value, as rendered by `encoding/json`. Object metadata
...
/// legacy `t.Format(time.RFC3339)` for a UTC instant.
...
/// day overflow like legacy `time.Time.AddDate(years, 0, 0)`, and returns RFC3339
...
/// (midnight UTC) — to UNIX seconds. Mirrors legacy `parseLifecycleExpirationDate`.
