//! legacy-compatible JSON encoding for on-disk metadata.
//!
//! The legacy S3 store persists every metadata file (`bucket.json`, `object.json`,
...
//!
//!  1. **2-space indentation** — `serde_json::to_vec_pretty` matches legacy
//!     `MarshalIndent(_, "", "  ")` (same `": "` separators, same per-element
//!     lines, same `{}`/`[]` for empties).
//!  2. **HTML escaping** — legacy escapes `<`, `>`, `&` and the U+2028/U+2029 line
...
//!     `BTreeMap` keys sort (matching legacy struct-vs-map split).

use serde::Serialize;

/// Rewrites `<`, `>`, `&` and U+2028/U+2029 to their `\uXXXX` escapes, matching
/// legacy `encoding/json` HTML escaping. Operates on valid JSON — these bytes only
...
/// Indented JSON + legacy HTML escaping + trailing `\n`, matching the legacy store's
...
/// Compact JSON + legacy HTML escaping, no trailing newline — matches `json.Marshal`
