...
//!
//! Field order and `serde` attributes reproduce the legacy `encoding/json` output
//! byte-for-byte:
//!
//!  - Fields are declared in the **same order** as the legacy structs in `types.rs`
//!    (serde derive preserves declaration order).
//!  - legacy `omitempty` on a **string / int / bool / map / pointer** is reproduced
//!    with `skip_serializing_if`. legacy `omitempty` on a **struct** field is a
...
//!    RFC3339Nano strings and always emit (legacy `time.Time` ignores `omitempty`).
//!  - `#[serde(default)]` at the container level makes decoding tolerant of
//!    missing keys, matching `json.Unmarshal`.
//!  - The legacy structs' `xml.Name` fields are `json:"-"` and are omitted here; the
...
/// The version ID legacy writes for objects in a versioning-suspended bucket.
