//! The on-disk `FileBucketStore` — the `BucketStore` boundary the legacy S3 service
...
/// Mirrors the allowlist in the legacy `DeleteBucket`.
...
/// A `FileBucketStore` error. The legacy store returns `error`; callers distinguish
...
impl FileBucketStore {
    ...
    /// 16 random bytes (matching the legacy `newUploadID`/`newVersionID` format).
    ...
    /// bucket directory already existed (matching the legacy `CreateBucket`).
    pub fn create_bucket(&self, name: &str) -> Result<(Bucket, bool)> {
        ...
        match fs::metadata(&path) {
            Ok(_) => {
                // Directory exists: mirror legacy `return bucket, !ok, err`.
