//! Mirrors `internal/services/applicationautoscaling/{server,store,persistence,handlers}.rs`.
//!
//! In-memory state behind a `Mutex`, persisted to `state.json` (byte-compatible
//! with the legacy server). Each operation returns an [`Outcome`] (status + optional
...
/// Server configuration. Mirrors legacy `Config`.
...
pub struct Server {
    config: Config,
    state: Mutex<State>,
    /// Set when initial load failed, mirroring legacy `loadErr` → 500 on every op.
...
/// A v4-shaped UUID. The legacy server uses crypto randomness, but the value is not
...
/// Builds an AWS-shaped error body, mirroring legacy `writeError`.
