//! Minimal HTTP/1.1 front-end mirroring `routes.rs`'s single `/` POST handler.
//!
//! A hand-rolled HTTP/1.1 reader/writer on plain tokio (no framework) keeps the
//! dependency surface tiny. It replicates the legacy request gate exactly: only
...
async fn write_response(stream: &mut TcpStream, outcome: Outcome) -> std::io::Result<()> {
    // legacy json.Encoder appends a trailing newline; match it byte-for-byte.
