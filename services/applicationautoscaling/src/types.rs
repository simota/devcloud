//! Mirrors `internal/services/applicationautoscaling/types.rs`.
//!
//! Wire serialization is pinned to legacy `encoding/json`: struct fields keep
...
//! match legacy sorted-key map marshaling.
...
pub struct ScalableTarget {
    ...
    pub RoleARN: String,
    // legacy tags this `omitempty`, but `encoding/json`'s omitempty has NO effect on
    // struct values (only nil pointers / empty maps/slices / zero scalars), so
    // legacy always emits `"SuspendedState":{}`. Match that — do NOT skip it.
    #[serde(default)]
    pub SuspendedState: SuspendedState,
    /// RFC 3339 UTC string (legacy `time.Time`).
...
// --- Request types (all fields optional on the wire; mirror legacy pointer/zero
...
// --- Persistence schema (mirrors legacy `persistedState`) ---
