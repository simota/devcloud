...
//!
//! Mirrors `internal/services/dynamodb/{routes,server}.go`: a single `POST /`
...
//! typed request, runs the op, and renders the result (or error) with Go's
...
impl Server {
    ...
    pub fn dispatch(
        ...
        // Expire TTL items before serving (mirrors the Go middleware).
...
fn decode<T: DeserializeOwned>(body: &[u8]) -> Result<T, Outcome> {
    // An empty body decodes to the type's default (Go decodes `{}` to zero value).
