...
//!
//! Mirrors the non-handler logic of `internal/services/dynamodb/streams.go`.
...
/// Go `streamIterator` (`streamArn`, `shardId`, `position`).
...
pub fn build_stream_record(
    ...
    // (Go computes it before setting the field). Uses Go-compatible marshaling.
    image.size_bytes = crate::go_json::marshal(&image).len() as i64;
    Some(StreamRecord {
...
/// Encodes an iterator to base64url without padding (Go `RawURLEncoding`).
pub fn encode_iterator(iterator: &StreamIterator) -> String {
    let payload = crate::go_json::marshal(iterator);
