...
pub struct ApiError {
    ...
    pub name: String,
    pub message: String,
    /// Extra top-level body fields merged with `__type`/`message`. Go builds the
...
impl ApiError {
    ...
    /// The JSON error body Go emits: `__type` + `message` plus any `extra`
    /// fields, as a `serde_json::Value` (sorted-key object, matching Go's
    /// `map[string]any`).
