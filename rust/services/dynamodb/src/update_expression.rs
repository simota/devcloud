...
//!
//! Mirrors `internal/services/dynamodb/{update_expressions,expression_tokens}.go`.
//! Operates in place on a mutable item (the merged key + existing attributes),
//! exactly as Go's `applyUpdateExpression` does. Number arithmetic delegates to
//! `crate::number` (Go's `big.Rat` semantics).
...
// --- tokenizers (mirror expression_tokens.go) ------------------------------
