...
//! DeleteItem) against golden oracles captured from the Go service.
...
#[test]
fn put_then_state_matches_go_oracle() {
    let dir = tempdir();
    ...
    assert_eq!(
        ...
        "state.json after PutItem must match the Go oracle byte-for-byte"
    );
}
...
fn put_returns_empty_and_all_old() {
    ...
    assert_eq!(
        ...
        String::from_utf8_lossy(&want),
        "PutItem ALL_OLD body must match the Go oracle"
    );
}
...
fn get_full_projection_and_missing() {
    ...
    assert_eq!(
        ...
        String::from_utf8_lossy(&want),
        "GetItem body must match the Go oracle"
    );
    ...
    assert_eq!(
        ...
        "GetItem projection body must match the Go oracle"
    );
...
fn delete_condition_failure_and_all_old() {
    ...
    assert_eq!(
        String::from_utf8_lossy(&body),
        String::from_utf8_lossy(&want),
        "DeleteItem ALL_OLD body must match the Go oracle"
    );
...
#[test]
fn put_validation_errors_match_go() {
    let dir = tempdir();
    let mut s = server(&dir);
    create_t(&mut s);
