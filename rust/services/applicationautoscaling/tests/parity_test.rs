//! 1:1 parity port of `internal/services/applicationautoscaling/server_test.go`,
//! plus byte-exact golden-oracle assertions captured from the Go server.
...
/// Mirrors the Go test helper `doRequest`: POST with the AWS JSON content-type
...
// --- Byte-exact golden oracle (captured from the Go server) ---

#[test]
fn error_body_matches_go_byte_for_byte() {
    ...
    // Go's writeError marshals a map (sorted keys) → __type before message, then
...
#[test]
fn empty_success_body_matches_go() {
...
#[test]
fn state_json_matches_go_byte_for_byte() {
    ...
    // match the Go server byte-for-byte — note `"SuspendedState":{}` is present
    // (Go's omitempty does not drop struct values) and there is no trailing
    // newline (Go uses json.Marshal + os.WriteFile, not an Encoder).
