//! Mirrors the `Store` interface from `internal/services/mail/service.go` and
//! the JSONL-backed `FileStore` from `internal/storage/mailstore/store.go`.
//!
...
//! format is byte-compatible with the Go store so the Go dashboard can read what
...
/// Storage backend for received messages. Mirrors Go `mail.Store`.
///
/// Methods are synchronous (blocking) — matching the Go store, whose operations
...
// ---------------------------------------------------------------------------
// RecordingStore — in-memory test/transient store (parity of Go recordingStore)
// ---------------------------------------------------------------------------

/// In-memory store that records appended messages and raw bodies, for tests and
/// transient runs. Parity counterpart of the Go test `recordingStore`.
#[derive(Default)]
...
impl RecordingStore {
    ...
    /// Snapshot of stored (messages, raw bodies). Mirrors the Go test helper.
...
// ---------------------------------------------------------------------------
// FileStore — JSONL-backed persistent store (parity of Go mailstore.FileStore)
// ---------------------------------------------------------------------------
...
pub struct FileStore {
    ...
    /// Serializes all file access, mirroring the Go store's `sync.Mutex`.
    lock: Mutex<()>,
}
...
impl FileStore {
    ...
    /// via a temp file + atomic rename. Mirrors Go `FileStore.rewrite`.
