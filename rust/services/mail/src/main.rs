...
//!
//! The Go daemon launches this as a subprocess on the same `127.0.0.1:<port>`
//! the in-process Go SMTP server would have used, pointed at the same storage
//! paths. Because the Rust `FileStore`/`FileBlobStore` write a byte-compatible
//! on-disk format, the Go dashboard keeps reading messages transparently.
//!
...
//! Go server's context-cancellation shutdown.
...
/// Resolves when SIGINT or SIGTERM arrives, mirroring the Go daemon's
