//! Mirrors `internal/services/mail/parser.go`.
//!
//! Go uses `net/mail` + `mime` + `mime/multipart`. We hand-roll the minimal
...
fn fill_message_body(msg: &mut Message, content_type: &str, body: &[u8]) {
    ...
    // Unparseable Content-Type → default to text body (mirrors Go's err==nil gate
