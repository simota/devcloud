//! Mirrors `internal/services/mail/smtp.go`.
//!
...
//! the Go server: same reply codes, same sequence checks, same DATA framing
...
impl SmtpServer {
    ...
    /// drive it over an in-memory duplex (the parity of Go's `net.Pipe`).
