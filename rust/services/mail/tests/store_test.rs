//! Parity port of `internal/storage/mailstore/store_test.go`, plus byte-exact
//! golden-oracle assertions captured from the Go `FileStore` so the on-disk
//! format stays interoperable with the Go dashboard during the strangler-fig
//! window.
...
fn file_store_list_returns_empty_slice_when_log_missing() {
    ...
    // Go's TestFileStoreListReturnsEmptySliceWhenLogMissing requires the result
...
// --- Byte-exact golden oracle (captured from the Go FileStore) ---

#[test]
fn file_store_jsonl_matches_go_byte_for_byte() {
    ...
    // Go golden line (note trailing newline appended by the encoder).
    ...
    assert_eq!(
        got, want_line,
        "JSONL must match Go encoding/json byte-for-byte"
    );

    // Blob is sharded by the Go content-addressed scheme: <a>/<b>/<id>.blob.
    ...
    assert!(
        want_blob.exists(),
        "blob not at Go shard path: {want_blob:?}"
    );
    assert_eq!(
