...
//! rebuild), and prefix listings. Ported 1:1 from the Go `store_objects.go`.
...
/// to `default_vid`. Mirrors the Go `readObjectMetadataForVersionList`.
...
impl FileBucketStore {
    ...
    /// marker (matching the Go `GetObject`).
