//! Deterministic checksum/ETag helpers matching the Go S3 store byte-for-byte:
...
/// CRC32C (Castagnoli) of `data`, big-endian, base64-StdEncoding — matching Go's
...
/// `Content-MD5` validation failure, mirroring the Go sentinel errors.
...
/// no-op (matching the Go `validateContentMD5`).
