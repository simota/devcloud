...
//! and the object-lock decision/derivation functions, ported from the Go
//! `store.go` helpers.
...
/// Input to `FileBucketStore::put_object` (the Go `PutObjectInput`, with the body
...
/// Lowercases metadata keys; an empty map stays empty (the Go `cleanMetadata`
...
/// (RFC3339(/Nano) UTC). Mirrors the Go `objectLockPreventsDelete`.
...
/// object-lock configuration. `now` is RFC3339(/Nano) UTC. Mirrors the Go
...
/// maps to the literal `null`. Mirrors the Go `objectVersionID`.
