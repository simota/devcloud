...
//! (delete markers, version listings, current-object rebuild) reproduce the Go
//! store behavior captured in `/tmp/s3_oracle_objects.txt`, and the delete-marker
//! `object.json` matches the Go zero-time render byte-for-byte.
...
fn delete_marker_json_matches_oracle() {
    // A delete marker leaves createdAt at the Go zero time and always emits the
