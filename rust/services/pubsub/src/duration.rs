//! Go `time.ParseDuration` reproduction, used to validate `messageRetentionDuration`,
//! `expirationPolicy.ttl`, etc.
//!
//! Mirrors `parseGoogleDuration` in `internal/services/pubsub/validation.go`,
//! which calls `time.ParseDuration` and rejects negatives. Go accepts a signed
...
/// Parses a Go duration string, returning the total nanoseconds. Returns `None`
...
/// True when `value` is a valid non-negative Go duration (the
