...
//!
//! Field declaration order and `omitempty` mirror the Go structs in
//! `internal/services/pubsub/types.go`, because `resources.json` / `pubsub.json`
...
//! `serde_json::Value` (sorted-key objects), matching Go's `map[string]any`.
...
/// A per-subscription delivery record. Go has **no** JSON tags on
/// `deliveryRecord`, so the field names are PascalCase and times are RFC3339
/// strings (Go marshals `time.Time` as RFC3339). The Go zero time marshals as
...
/// Go's zero `time.Time` RFC3339 rendering.
