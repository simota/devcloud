//! `resources.json` + `pubsub.json` persistence — byte-compatible with the Go
//! service.
//!
//! Mirrors `resourceFile` / `messageStateFile` in
//! `internal/services/pubsub/types.go`. Both are written with
...
impl ResourceFile {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        crate::go_json::to_vec_indent(self)
    }
}
...
mod tests {
    ...
    fn topic_resources_round_trip_byte_for_byte() {
        ...
        assert_eq!(
            file.to_bytes(),
            fixture.to_vec(),
            "re-encoded resources.json must match the Go oracle byte-for-byte\n--- got ---\n{}\n--- want ---\n{}",
