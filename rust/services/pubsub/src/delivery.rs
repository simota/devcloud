...
//! `internal/services/pubsub/{message_handlers,retention_leases}.go`. Delivery
//! timestamps are RFC3339 strings; the Go zero time is `0001-01-01T00:00:00Z`.
//! These helpers compare/add seconds in UNIX space.

use crate::model::ZERO_TIME;

/// True for the Go zero time (`0001-01-01T00:00:00Z`) or an empty string.
...
/// True when `value` is strictly after `now_secs` (Go's `t.After(now)`); the
