...
//! tags, policy), with values captured from the Go server.
...
#[test]
fn url_arn_and_defaults_match_go() {
...
#[test]
fn queue_name_from_url_matches_go() {
    assert_eq!(
...
fn computed_attributes_present() {
    ...
    // CreatedTimestamp is a positive UNIX time (now), not the Go zero time.
...
#[test]
fn add_permission_policy_json_matches_go() {
    let mut s = Server::new(cfg());
    let url = "http://127.0.0.1:9324/000000000000/Orders";
    s.create_queue("Orders", &BTreeMap::new(), &BTreeMap::new())
        .unwrap();
    ...
    let attrs = s
        ...
        .unwrap();
    // Byte-exact match to the Go json.Marshal(queuePolicy) output.
...
#[test]
fn normalized_permission_actions_matches_go() {
    let actions = vec![
        "SendMessage".to_string(),
        "sqs:Recv".to_string(),
        "*".to_string(),
