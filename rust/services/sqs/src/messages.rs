...
//! `internal/services/sqs/{message_handlers,message_core}.go`:
...
//! immediately, matching the Go fast path. The Go wait-channel is purely an
...
/// Mirrors Go `receivedMessage` (response projection).
...
impl Server {
    // --- send (mirror message_handlers.go sendMessage) ---
    ...
    // --- receive (mirror message_core.go) ---
    ...
    pub fn receive_messages(
        ...
        let attempts = wait_seconds * 10 + 1; // ~100ms slices, mirroring the Go loop
    ...
    // --- delete / change visibility (mirror message_core.go) ---
...
// --- free helpers (mirror message_core.go + queue_attributes.go) ---
...
/// Mirrors `newOpaqueID`: `<prefix>-<random>`. The Go id uses crypto randomness
