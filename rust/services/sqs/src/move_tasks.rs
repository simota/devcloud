//! Mirrors `internal/services/sqs/deadletter_move_tasks.go` +
//! `resolveOriginalSourceLocked` (from dashboard.go): dead-letter source-queue
...
/// A single move-task result projection (mirrors Go `messageMoveTaskResult`).
...
impl Server {
    ...
    pub fn start_message_move_task(
        ...
        // Mirrors Go exactly: no retention cleanup here; moved messages are
    ...
    pub fn list_message_move_tasks(
        ...
        // Mirrors Go: maxResults defaults to and is capped at 10.
    ...
    /// task. (The Go server does not change task status on cancel.)
