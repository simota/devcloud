...
//! queue_attributes,tags_policy,persistence}.go`.
//!
...
//! the Go wording verbatim (the responses layer maps message substrings to AWS
//! error codes).
...
/// Mirrors Go `Config` (queue-management subset; message/scheduler fields land
/// with later parts).
...
/// In-memory queue state. Mirrors Go `queueState`; timestamps are RFC 3339
...
impl Server {
    ...
    // --- config getters (mirror persistence.go) ---
    ...
    // --- URL / ARN (mirror queue_handlers.go) ---
    ...
    // --- queue lifecycle (mirror queue_handlers.go) ---
    ...
    pub fn list_queue_urls(&self, prefix: &str) -> Vec<String> {
        // BTreeMap iterates sorted, matching Go's sort.Strings(names).
        self.queues
            .iter()
    ...
    // --- attributes (mirror queue_attributes.go) ---
    ...
    // --- policy / permissions (mirror tags_policy.go) ---
    ...
    // --- persistence (mirror persistence.go) ---
...
pub fn queue_name_from_url(queue_url: &str) -> String {
    ...
    // there are at least two path segments (matching Go's url.Parse + trim).
...
fn queue_attributes_with_computed(queue: &QueueState) -> BTreeMap<String, String> {
    ...
    // Counts are 0 in this part (message lifecycle lands later); the Go server
