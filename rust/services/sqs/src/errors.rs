//! Mirrors `errorCode` from `internal/services/sqs/responses.go`: maps an error
...
/// Maps a Go-style error message to its AWS error code via substring match,
/// in the same priority order as the Go `errorCode` switch.
