//! Mirrors `internal/services/sqs/sigv4.go`.
//!
//! AWS SigV4 verification for the SQS service (`service = "sqs"`), with the same
//! error codes + HTTP statuses the Go server returns. Relaxed mode (default)
...
/// A signature failure carrying the AWS error code + HTTP status from the Go
/// server.
