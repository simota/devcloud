...
`services/dynamodb` は DynamoDB の table、item、index、stream semantics を Rust crate 内に閉じ込める。Dashboard 連携は `services/dashboard` の DynamoDB handler が HTTP/API 境界で扱う。
