...
- AWS SDK for Go / JavaScript / Python の基本操作が endpoint override だけで通る。
...
`services/object` は provider-neutral な object core とする。将来 GCS 互換 API を実装する場合も同じ core を使う。
...
4. AWS SDK for Go v2, boto3, and AWS SDK for JavaScript v3 pass smoke tests with endpoint override.
