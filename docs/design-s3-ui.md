...
v0.1 can remain Rust-served static HTML/CSS/JS, but future multi-service expansion should move S3 into the shared React/Vite dashboard shell. Production runtime must remain a single Rust binary that serves embedded built assets and does not require Node.js.

### v0.1 Static Dashboard

Use the same production direction as Mail:

- Rust-served static HTML/CSS/JS
...
`mock/s3/` is the interactive reference artifact for this design. It should be used to validate layout, density, empty states, dialogs, and responsive behavior before implementing the Rust-served dashboard.
