...
internal/services/sqs/
  server.go              # HTTP server and route dispatch
  protocol.go            # Query and AWS JSON protocol codecs
  operations.go          # operation handlers
  errors.go              # SQS-compatible error responses
  sigv4.go               # AWS SigV4 verifier, shared shape with S3/DynamoDB
  model.go               # SQS request/response/resource structs
  attributes.go          # queue/message attribute validation
  md5.go                 # SQS MD5 calculation helpers
  fifo.go                # FIFO ordering and dedup helpers
  server_test.go
  protocol_test.go
  scheduler_test.go

internal/storage/message/
  store.go               # queue and message persistence interface
  fs.go                  # filesystem-backed implementation
  wal.go                 # append/replay crash recovery
  lease.go               # visibility timeout receipt leases
  scheduler.go           # delay/visibility/retention timers
  store_test.go

internal/app/
  config.go              # server.sqsPort, auth.sqs, services.sqs
  daemon.go              # SQS lifecycle wiring

internal/dashboard/
  server.go              # service registry + /api/sqs/*
  services.go            # SQS dashboard status metadata
...
- IMPL-002: SQS API contract fixtures を `internal/services/sqs/testdata` に追加する。
...
- IMPL-010: `internal/services/sqs` package を追加する。
...
- AWS SDK for Go v2 client smoke
- boto3 client smoke
...
3. Should `internal/services/s3/sigv4.go` and `internal/services/dynamodb/sigv4.go` be extracted before SQS, or should SQS start with a local copy and refactor later?
...
6. Should Message Core be introduced as `internal/storage/message` immediately, or start under `internal/services/sqs` and extract before Pub/Sub?
