...
`devcloud` provides a local Redis surface by managing a real `redis-server` child process (or proxying to a user-supplied external Redis), then exposing a `/dashboard/redis` management UI. The daemon does **not** re-implement RESP. Protocol parity is delegated to upstream `redis-server`; devcloud owns lifecycle, configuration, and management UX. This is the same `managed`/`external` split already established for Redshift in `internal/app/managed_postgres.go`.
...
internal/services/redis/
  server.go        Server, NewServer(Config), Run(ctx)
                   - managed: lifecycle + go-redis health probe
                   - external: connection validation
  config.go        Config, AuthMode, defaults
  client.go        in-process *redis.Client wrapper (go-redis v9)
  snapshot.go      Status / KeysSnapshot / KeyDetailSnapshot for dashboard
  command_allowlist.go    allowed commands + classification (read vs destructive)
  *_test.go        per-file tests (config validation, allowlist, snapshot JSON shape)

internal/app/
  managed_redis.go redis-server child process lifecycle (SIGTERM -> wait -> SIGKILL)
```

## 5. Daemon Wiring

Mirror the Redshift integration shape in `internal/app/daemon.go`:
...
### Backend (`internal/dashboard/redis_handlers.go`)
...
|------------|------|------|
| `github.com/redis/go-redis/v9` | Go module (new) | Dashboard in-process client only |
| `redis-server` binary | external runtime | Required for `managed` mode |

`go-redis` is the de-facto Go client; pinning to a `v9.x` minor stream is fine.

## 8. Tests

| Layer | Location | Coverage |
|-------|----------|----------|
| Unit | `internal/services/redis/server_test.go` | Config validation, allowlist enforcement, status snapshot shape |
| Managed lifecycle | `internal/app/managed_redis_test.go` | binary-missing error, graceful shutdown, port collision |
| Dashboard | `internal/dashboard/redis_test.go` | all endpoints, empty-keyspace `[]` shape, disallowed command rejection |
| Integration | `internal/services/redis/integration_test.go` | gated on `redis-server`-in-PATH, real TCP via go-redis |
...
| go-redis breaking change | pin to `v9.x` minor; bump deliberately via Shift |

## 11. References

- `internal/app/managed_postgres.go` — managed child process pattern
- `internal/services/redshift/server.go` — managed/external split
- `internal/dashboard/redshift_handlers.go` — handler / in-process service wiring
