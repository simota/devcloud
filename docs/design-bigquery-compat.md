...
services/
  ...
  services/
    bigquery/
      server.rs
      router.rs
      handlers_projects.rs
      handlers_datasets.rs
      handlers_tables.rs
      handlers_tabledata.rs
      handlers_jobs.rs
      handlers_upload.rs
      auth.rs
      errors.rs
...
`storage/bigquery` は BigQuery の catalog/table/job semantics を閉じ込める。load/extract job は将来的に `services/gcs` / `services/s3` の object core と連携する。
...
| --- | --- | --- |
| `STRING` | compatibility `string` | Yes |
...
```txt
GET /bigquery             service page entry, returns the shared dashboard app
...
2. create/list/get a dataset through `first-party BigQuery SDK`.
...
| Client library endpoint override differs by language | Medium | Test compatibility / Python / Node client smoke where practical |
