...
- REQ-005b: PostgreSQL backend を primary SQL execution engine とし、現在の Go 製簡易 executor は migration 中の fallback / contract test fixture として扱えるようにする。
...
| --- | --- | --- |
| Daemon wiring | `internal/app` | config load, listener startup, reset integration |
| SQL listener | `internal/services/redshift/pgwire` | PostgreSQL wire protocol startup, auth, query messages |
| HTTP API | `internal/services/redshift/api` | AWS Redshift Data API / management API request parsing and responses |
| Service facade | `internal/services/redshift` | shared business operations across SQL, Data API, dashboard |
| Metadata store | `internal/storage/redshift/catalog` | Redshift-only metadata, cluster metadata, dist/sort/encode attributes, statement history |
| PostgreSQL backend | `internal/services/redshift/backend/postgres` | SQL execution, transaction behavior, base catalog, result sets |
| Fallback backend | `internal/services/redshift/backend/memory` | migration-only compatibility fixture for tests and no-postgres development |
| Statement store | `internal/storage/redshift/statement` | Data API statement lifecycle, result pages, sessions |
| Redshift translator | `internal/services/redshift/translator` | Redshift SQL parsing, function rewrite, attribute extraction, PostgreSQL SQL generation |
| Side effect router | `internal/services/redshift/sideeffects` | COPY / UNLOAD / system view / metadata-only operations handled by devcloud |
| Dashboard API | `internal/dashboard` | Redshift route, status, query history, resource operations |
...
- IMPL-005: Add Redshift config fields in `internal/app/config.go`.
- IMPL-006: Start SQL and HTTP API listeners from `internal/app/daemon.go`.
...
- IMPL-027: Add AWS CLI / Go SDK smoke tests.
...
- IMPL-045: Move the current Go SQL executor behind `backend/memory` and keep it as a fallback fixture.
...
The current Redshift implementation passes the local full gate with a Go-based SQL executor and filesystem-backed state. This is acceptable as a compatibility scaffold, but it should not remain the long-term database engine because SQL grammar, transaction semantics, catalog behavior, and client introspection will expand quickly.
