...
- SDK smoke tests for compatibility, Node.js, and Python where dependency footprint is acceptable.
...
```txt
services/pubsub/
  server.rs              # service composition and lifecycle
  config.rs              # Pub/Sub service config normalization
  grpc_server.rs         # gRPC listener and registration
  rest_server.rs         # REST v1 endpoint dispatch
  publisher.rs           # topic operations and publish
  subscriber.rs          # subscription operations and pull/ack
  streaming_pull.rs      # StreamingPull implementation
  push_worker.rs         # push subscription worker
  validation.rs          # resource names, field ranges, request validation
  errors.rs              # gRPC and REST error mapping
  types.rs               # adapter-level domain types

services/message/
  core.rs                # Message Core interfaces shared by SQS and Pub/Sub
  fs_store.rs            # filesystem-backed message storage
  scheduler.rs           # lease expiration and redelivery scheduler
  clock.rs               # deterministic test clock

services/pubsub/
  store.rs               # Pub/Sub resource metadata store
  fs_store.rs
  snapshots.rs
  schemas.rs
...
Generated protobuf code should be added only if needed. Prefer importing official `tonic-generated Pub/Sub protobuf service bindings` and implementing the generated interfaces instead of checking in generated source.
...
- IMPL-005: Add Pub/Sub config fields in `orchestrator/config.rs`.
- IMPL-006: Start gRPC and REST listeners from `orchestrator/daemon.rs`.
...
- IMPL-014: Extract or finalize `services/message` interfaces from SQS work.
...
- IMPL-021: Add Google Cloud first-party client smoke test.
...
- AC-010: `cargo test --workspace` and `VERIFY_STAGE=full bash scripts/pubsub-autoloop/verify.sh` pass.
...
5. Should Message Core live in `services/message` immediately, or remain under SQS until Pub/Sub Phase 3 forces extraction?
